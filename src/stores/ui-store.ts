  ...
  devtools(
    persist(
      (set) => ({
        ...
        closeAddDialog: () =>
          set({ showAddDialog: false, addDialogPrefilledData: null }, false, 'closeAddDialog'),
        openSettingsDialog: (section?: string) =>
          set(
        ...
        setJournalUnreadCount: (count: number) =>
          set({ journalUnreadCount: count }, false, 'setJournalUnreadCount'),

        // 过滤操作
        setActiveTab: (activeTab) => set({ activeTab }, false, 'setActiveTab'),
        setSelectedDayFilter: (selectedDayFilter) =>
          set({ selectedDayFilter }, false, 'setSelectedDayFilter'),
