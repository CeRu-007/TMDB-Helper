...
export default function RobustCSVProcessor() {
  ...
  const deleteEpisodes = (
    ...
    const episodesToDeleteSet = new Set(
      episodesToDelete.map((ep) => String(ep)),
    );
      ...
      {processedData && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>显示处理后的前5行数据</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {processedData.rows.slice(0, 5).map((row, rowIndex) => (
                    <tr key={rowIndex} className="hover:bg-gray-50">
                      {row.map((cell, cellIndex) => (
                        <td
                          key={cellIndex}
                          className="border border-gray-300 px-2 py-1 text-sm max-w-xs truncate"
                        >
                          {cell}
                        </td>
                      ))}
