...
export function TimePickerPresets({ onSelectPreset, className }: TimePickerPresetsProps) {
  // 预设时间列表
    ...
    <div className={cn('flex flex-wrap gap-2', className)}>
      {presets.map((preset) => (
        ...
        >
          ...
          {preset.label === '当前'
            ? preset.label
            : `${preset.label} ${preset.hour.toString().padStart(2, '0')}:${preset.minute.toString().padStart(2, '0')}`}
        </Button>
      ))}
    </div>
  );
