...
const TimeWheelPicker = ({
  ...
  const updateTime = useCallback(
    (newHour: number, newMinute: number) => {
      onTimeChange((newHour + 24) % 24, (newMinute + 60) % 60);
    },
    [onTimeChange]
  );

  // 处理移动增量
  const handleDeltaMovement = useCallback(
    (delta: number, type: 'hour' | 'minute') => {
      if (Math.abs(delta) < 10) {
        return;
      } // 需要足够的移动才改变值

      if (type === 'hour') {
        const change = delta > 0 ? -1 : 1; // 向上滑动减少，向下滑动增加
        ...
        const change = delta > 0 ? -minuteStep : minuteStep;
  ...
  const applyInertia = useCallback(() => {
    if (Math.abs(velocity) < 3 || activeWheel === null) {
      return;
    }
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      if (activeWheel === null || e.touches.length !== 1) {
        return;
      }
  ...
  const handleWheelMouseDown = useCallback(
    (e: React.MouseEvent, type: 'hour' | 'minute') => {
      e.preventDefault();
  ...
  const handleWheelTouchStart = useCallback(
    (e: React.TouchEvent, type: 'hour' | 'minute') => {
      e.preventDefault();
  ...
  const handleButtonClick = useCallback(
    (type: 'hour' | 'minute', direction: 'up' | 'down') => {
  ...
  const renderWheelOptions = (options: number[], currentValue: number) => {
    ...
    return extendedOptions.slice(startIndex, startIndex + visibleOptions).map((value, index) => {
    ...
    <div className={cn('flex gap-4 items-center justify-center', className)}>
      ...
      <div className="relative flex flex-col items-center">
        ...
        >
          ...
          {/* 选项列表 */}
          <div className="flex flex-col">{renderWheelOptions(hours, hour)}</div>
        </div>
      ...
      <div className="relative flex flex-col items-center">
        ...
        >
          ...
          {/* 选项列表 */}
          <div className="flex flex-col">{renderWheelOptions(minutes, minute)}</div>
        </div>
