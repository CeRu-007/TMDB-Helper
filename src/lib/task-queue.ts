...
export class TaskQueue {
  ...
  public async enqueue(
    ...
    console.log(`[TaskQueue] 任务已加入队列: ${task.name} (优先级: ${priority})`);
  ...
  private async processQueue(): Promise<void> {
    try {
      ...
      const queuedTasks = Array.from(this.queue.values())
        .filter(t => t.status === 'queued' || (t.status === 'failed' && this.shouldRetry(t)))
  ...
  private async processTask(queuedTask: QueuedTask): Promise<void> {
    ...
    const lockResult = await DistributedLock.acquireLock(lockKey, 'task_execution', 15 * 60 * 1000);
    ...
    try {
      ...
      console.log(`[TaskQueue] 开始处理任务: ${queuedTask.task.name} (尝试 ${queuedTask.attempts}/${queuedTask.maxAttempts})`);

      // 开始性能监控
      performanceMonitor.startTaskExecution(queuedTask.id, queuedTask.task.name);
      ...
      console.log(`[TaskQueue] 任务处理完成: ${queuedTask.task.name} (耗时: ${duration}ms)`);
      ...
      if (queuedTask.attempts < queuedTask.maxAttempts) {
        ...
        queuedTask.nextRetryAt = this.calculateNextRetry(queuedTask.attempts).toISOString();
        console.log(`[TaskQueue] 任务将重试: ${queuedTask.task.name} (${queuedTask.attempts}/${queuedTask.maxAttempts})`);
      } else {
        queuedTask.status = 'failed';
      ...
      // 结束性能监控
      performanceMonitor.endTaskExecution(queuedTask.id, 'failed', queuedTask.error);
   ...
   * 清理已完成的任务
   */
  public async cleanupCompletedTasks(olderThanHours: number = 24): Promise<number> {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        ...
        if (queueData) {
          ...
          if (parsed.tasks && Array.isArray(parsed.tasks)) {
            ...
            console.log(`[TaskQueue] 从存储加载了 ${this.queue.size} 个队列任务`);
          }
        }
      }
    } catch (error) {
   ...
   * 更新任务优先级
   */
  public async updateTaskPriority(queueId: string, priority: QueuedTask['priority']): Promise<boolean> {
