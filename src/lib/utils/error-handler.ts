...
export class ExternalServiceError extends ApplicationError {
  constructor(service: string, message?: string) {
    super(message || `${service}服务暂时不可用`, ErrorCode.EXTERNAL_SERVICE_ERROR, 503, true, {
      service,
    });
  }
}
...
class ClientErrorHandler {
  ...
  private isApiError(error: unknown): boolean {
    ...
    return !!e.response || !!e.status || !!e.statusCode;
  }
  ...
  private isTimeoutError(error: unknown): boolean {
    ...
    return !!(e.name === 'TimeoutError' || e.message?.includes('timeout') || e.code === 'TIMEOUT');
  }
  ...
  async retry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        logger.warn('ErrorHandler', `操作失败，${waitTime}ms后重试`, {
          attempt,
