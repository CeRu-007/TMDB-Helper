...
const log = new Logger({ prefix: 'API' });
...
class ApiClient {
  ...
  async get<T = any>(
    ...
    return this.request<T>(url, { ...options, method: 'GET', params, cache: undefined });
  }
  ...
  async delete<T = any>(url: string, options?: RequestInit): Promise<ApiResponse<T>> {
    return this.request<T>(url, { ...options, method: 'DELETE', cache: undefined });
  }
