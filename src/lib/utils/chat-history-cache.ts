...
class ChatSyncManager {
  ...
  async queueUpdate(chatHistory: ChatHistory): Promise<void> {
    ...
    // 防抖处理，避免频繁同步到服务器
    this.syncQueue = this.syncQueue.then(() => this.flushUpdates()).catch(logger.error);
  }
  ...
  async deleteChatHistory(chatId: string): Promise<void> {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const transaction = (this.cache as any).db!.transaction(
          [(this.cache as any).STORE_NAME],
          'readwrite'
        );
        const store = transaction.objectStore((this.cache as any).STORE_NAME);
