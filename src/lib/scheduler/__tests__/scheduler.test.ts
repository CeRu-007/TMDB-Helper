...
describe('Scheduler', () => {
  ...
  describe('任务执行', () => {
    ...
    it('executeTask 拒绝正在执行的任务', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        '[Scheduler] 任务正在执行中，跳过本次触发: task-running'
      );
    });
  });
  ...
  describe('cron 表达式验证', () => {
    it('接受有效的 cron 表达式', async () => {
      ...
      for (const cronExpr of validCrons) {
        const task = makeTask({
          id: `task-${cronExpr.replace(/[^a-z0-9]/gi, '-')}`,
          cron: cronExpr,
        });
        scheduler.scheduleTask(task);
        expect(scheduler.getScheduledTasks()).toContain(
          `task-${cronExpr.replace(/[^a-z0-9]/gi, '-')}`
        );
  ...
  describe('日志记录', () => {
    ...
    it('任务失败时更新日志状态', async () => {
      const { scheduleLogRepository } = await import('@/lib/data/schedule-log-repository');
      vi.mocked(scheduleLogRepository.updateStatus).mockReturnValue({ success: true } as any);
...
describe('TaskQueue', () => {
  ...
  describe('入队行为', () => {
    ...
    it('重复入队同一任务被检测', async () => {
      ...
      const { itemsRepository } = await import('@/lib/database/repositories/items.repository');
      vi.mocked(itemsRepository.findByIdWithRelations).mockReturnValue(undefined);
...
describe('桌面端多场景综合测试', () => {
  ...
  describe('场景1：窗口关闭后定时任务是否正常触发', () => {
    ...
    it('窗口多次关闭打开，任务不被重复调度', async () => {
      ...
      expect(scheduler.getScheduledTasks().filter((t) => t === 'scene1-no-duplicate')).toHaveLength(
        1
      );
    });
  });
...
describe('任务日志与监控测试', () => {
  ...
  describe('日志记录', () => {
    ...
    it('任务成功时更新日志状态为 success', async () => {
      ...
      await scheduleLogRepository.updateStatus('log-1', 'success', undefined);
  ...
  describe('任务执行记录', () => {
    it('记录任务最近一次执行时间', async () => {
      const { scheduleRepository } = await import('@/lib/data/schedule-repository');
      vi.mocked(scheduleRepository.updateLastRunAt).mockReturnValue({ success: true } as any);

      scheduleRepository.updateLastRunAt('task-1', expect.any(String), expect.any(String));
