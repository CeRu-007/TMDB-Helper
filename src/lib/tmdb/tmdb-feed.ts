...
export async function fetchTmdbFeed(
  ...
  const movieParams: Record<string, string | undefined> = {
    ...baseParams,
    region: regionStr,
    sort_by: kind === 'upcoming' ? 'release_date.asc' : 'release_date.desc',
  };

  const tvParams: Record<string, string | undefined> = {
    ...baseParams,
  ...
  const cleanParams = <T extends Record<string, string | undefined>>(
    p: T
  ): Record<string, string> =>
    Object.fromEntries(Object.entries(p).filter(([_, v]) => v !== undefined)) as Record<
      string,
      string
    >;

  const moviesResponse = await fetchTMDB('/discover/movie', cleanParams(movieParams), apiKey);
  await new Promise((resolve) => setTimeout(resolve, 100));
  const tvShowsResponse = await fetchTMDB('/discover/tv', cleanParams(tvParams), apiKey);
