...
export abstract class AuthenticatedAPIRoute extends BaseAPIRoute {
  ...
  public override async execute(
    ...
    try {
      const response = await this.handleAuthenticated(request, authResult.userId!, context);
  ...
  protected override handle(): Promise<NextResponse> {
    throw new Error(
      'Direct handle method called on AuthenticatedAPIRoute. Use handleAuthenticated instead.'
    );
  }
