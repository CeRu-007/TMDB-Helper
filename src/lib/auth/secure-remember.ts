...
export async function saveRemember(
  username: string,
  password: string,
  remember: boolean
): Promise<void> {
...
export async function loadRemember(): Promise<{
  username: string;
  password: string;
  remember: boolean;
}> {
  ...
  try {
    ...
    return {
      username: storageService.get<string>(USERNAME_KEY, ''),
      password: '',
      remember: storageService.get<string>(REMEMBER_KEY, '0') === '1',
    };
  }
}
