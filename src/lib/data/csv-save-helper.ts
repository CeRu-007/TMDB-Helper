...
import { CSVData, parseCsvContent, processOverviewColumn } from './csv-processor-client';
...
export function processOverviewData(data: CSVData): CSVData {
  ...
  const overviewIndex = data.headers.findIndex(
    (header: string) =>
...
export async function saveCSV(options: {
  ...
  try {
    ...
    const saveMessage =
      options.editorMode === 'table' ? '正在保存CSV文件...' : '正在保存文本内容...';
    ...
    if (options.editorMode === 'text' && options.csvContent) {
      ...
      if (!options.skipDataParsing && options.csvContent !== undefined) {
        try {
          ...
          throw new Error(
            `CSV文本格式有误，无法解析: ${error instanceof Error ? error.message : '未知错误'}`
          );
        }
