...
// import { StorageSyncManager } from './data/storage-sync-manager'; // 移除已删除的依赖
...
class TaskScheduler {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[TaskScheduler] 初始化完成，已加载 ${tasks.length} 个定时任务 (${enabledTasks.length} 个已启用)`,
      );
    } catch (error) {
      this.lastError =
  ...
  private async syncTasksToServer(tasks: ScheduledTask[]): Promise<void> {
    // API /api/sync-scheduled-tasks 已被删除，同步逻辑已集成到其他系统中
    // 直接返回，不执行任何操作
    return;
  }
    ...
    for (const task of enabledTasks) {
      try {
        ...
        if (timeDiff > 5 * 60 * 1000) {
          console.log(
            `[TaskScheduler] 发现错过的任务: ${task.name} (${task.id}), 预定时间: ${nextRunTime.toLocaleString('zh-CN')}, 当前时间: ${now.toLocaleString('zh-CN')}`,
          );
          ...
          if (timeDiff <= 24 * 60 * 60 * 1000) {
            console.log(
              `[TaskScheduler] 执行错过的任务: ${task.name} (${task.id})`,
            );
            ...
            console.log(
              `[TaskScheduler] 任务 ${task.name} (${task.id}) 错过时间过长 (${Math.round(timeDiff / (60 * 60 * 1000))} 小时)，跳过执行并重新调度`,
            );
  ...
  private calculateNextRunTime(task: ScheduledTask): Date {
    ...
    if (
      ...
      console.log(
        `[TaskScheduler] 计算每日任务下次执行时间: ${task.name} -> ${nextRun.toLocaleString('zh-CN')} (当前时间: ${now.toLocaleString('zh-CN')})`,
      );
      return nextRun;
    }
  }
  ...
  private calculateNextWeeklyRunTime(task: ScheduledTask, now: Date): Date {
    ...
    console.log(
      `[TaskScheduler] 计算每周任务下次执行时间: ${task.name} -> ${result.toLocaleString('zh-CN')} (当前时间: ${now.toLocaleString('zh-CN')}, 目标星期: ${targetDays.join(',')})`,
    );
    return result;
  }
  ...
  public scheduleTask(task: ScheduledTask): void {
    if (!task.enabled) {
      console.log(
        `[TaskScheduler] 任务 ${task.id} (${task.name}) 已禁用，不设置定时器`,
      );
      return;
    }
    ...
    if (conflicts.length > 0) {
      ...
      if (resolution) {
        if (resolution.strategy === 'queue') {
          ...
          console.log(
            `[TaskScheduler] 任务 ${task.name} 时间已调整: ${nextRunTime.toLocaleString('zh-CN')}`,
          );
        }
      }
    }
    ...
    const timer = setTimeout(async () => {
      console.log(
        `[TaskScheduler] 定时器触发: ${task.id} (${task.name}) 在 ${new Date().toLocaleString('zh-CN')}`,
      );
    ...
    console.log(
      `[TaskScheduler] 已为任务 ${task.id} 设置定时器，将在 ${nextRunLocale} 执行 (延迟 ${Math.round(adjustedDelay / 1000 / 60)} 分钟)`,
    );
  ...
  private scheduleTimerValidation(taskId: string, originalDelay: number): void {
    ...
    console.log(
      `[TaskScheduler] 为任务 ${taskId} 设置定时器验证，${Math.round(validationInterval / 60000)} 分钟后检查`,
    );
  }
    ...
    try {
      ...
      if (!this.timers.has(taskId)) {
        ...
        if (task && task.enabled) {
          ...
          console.warn(
            `[TaskScheduler] 无法重新设置定时器: 任务不存在或已禁用 (${taskId})`,
          );
        }
      } else {
        // 定时器存在，检查是否应该已经执行了
        ...
        if (task && task.nextRun) {
          ...
          if (timeDiff > 5 * 60 * 1000) {
            console.warn(
              `[TaskScheduler] 定时器异常: 任务 ${task.name} 应该在 ${nextRunTime.toLocaleString('zh-CN')} 执行，但现在是 ${now.toLocaleString('zh-CN')}`,
            );
            ...
            this.scheduleTimerValidation(
              taskId,
              Math.max(nextRunTime.getTime() - now.getTime(), 5 * 60 * 1000),
            );
          }
        }
      }
  ...
  private async getRelatedItem(task: ScheduledTask): Promise<TMDBItem | null> {
    try {
      ...
      if (foundItem) {
        console.log(
          `[TaskScheduler] 直接通过ID找到了项目: ${foundItem.title} (ID: ${foundItem.id})`,
        );
        return foundItem;
      }
      ...
      if (task.itemTmdbId) {
        ...
        if (tmdbMatch) {
          console.log(
            `[TaskScheduler] 通过TMDB ID找到了项目: ${tmdbMatch.title} (ID: ${tmdbMatch.id})`,
          );
      ...
      if (task.itemTitle) {
        ...
        if (titleMatch) {
          console.log(
            `[TaskScheduler] 通过标题精确匹配找到了项目: ${titleMatch.title} (ID: ${titleMatch.id})`,
          );
      ...
      if (task.itemTitle) {
        ...
        if (possibleItems.length > 0) {
          ...
          if (possibleItems.length === 1) {
            ...
            console.log(
              `[TaskScheduler] 选择唯一的模糊匹配项: ${matchItem.title} (ID: ${matchItem.id})`,
            );
          ...
          console.log(
            `[TaskScheduler] 从多个同类型候选项中选择创建时间最接近的: ${bestMatch.title} (ID: ${bestMatch.id})`,
          );
      ...
      if (task.itemId === '1749566411729') {
        ...
        console.log(
          `[TaskScheduler] 最近创建的项目: ${recentItems.map((item) => `${item.title} (ID: ${item.id}, 创建时间: ${item.createdAt})`).join(', ')}`,
        );
        ...
        if (recentItems.length > 0) {
          ...
          console.log(
            `[TaskScheduler] 应急修复: 将使用最近创建的项目 ${emergencyItem.title} (ID: ${emergencyItem.id}) 替代问题ID`,
          );
      ...
      if (nameMatchItems.length > 0) {
        ...
        if (nameMatchItems.length === 1) {
          ...
          console.log(
            `[TaskScheduler] 通过任务名称找到匹配项: ${nameMatch.title} (ID: ${nameMatch.id})`,
          );
        ...
        console.log(
          `[TaskScheduler] 从多个名称匹配项中选择创建时间最接近的: ${closestMatch.title} (ID: ${closestMatch.id})`,
        );
      ...
      if (sortedByDate.length > 0) {
        ...
        console.log(
          `[TaskScheduler] 备用策略: 使用最近创建的项目 ${lastResortItem.title} (ID: ${lastResortItem.id})`,
        );
      ...
      // 如果所有策略都失败，返回null
      console.warn(
        `[TaskScheduler] 无法找到任务 ${task.id} (${task.name}) 的关联项目，所有匹配策略均失败`,
      );

      // 创建一个特殊错误
      const error = new Error(
        `找不到项目ID ${task.itemId}，请尝试对任务进行重新关联或删除`,
      );
  ...
  private async executeTaskWithRetry(
    ...
    console.log(
      `[TaskScheduler] 准备执行任务: ${task.id} (${task.name}) 在 ${currentTime} (重试次数: ${retryCount})`,
    );

    // 如果任务已在执行中，跳过本次执行
    if (this.currentExecution.has(task.id)) {
      console.warn(
        `[TaskScheduler] 任务 ${task.id} (${task.name}) 已在执行中，跳过本次执行`,
      );
      // 重新调度任务
      this.scheduleTask(task);
      return;
    ...
    if (!task.enabled) {
      console.warn(
        `[TaskScheduler] 任务 ${task.id} (${task.name}) 已禁用，跳过执行`,
      );
      return;
    }
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        ...
        console.log(
          `[TaskScheduler] 将在 ${retryDelay / 60000} 分钟后重试任务: ${task.name} (第${retryCount + 1}次重试)`,
        );
        ...
        try {
          ...
          if (!relatedItem) {
            ...
            if (!result.success) {
              ...
              const items = await StorageManager.getItemsWithRetry();
              const fixResult = await StorageManager[
                'attemptToFixTaskAssociation'
              ](task, items);
              ...
              if (fixResult.success && fixResult.newItemId) {
                ...
                if (relatedItem) {
                  ...
                  throw new Error(
                    `修复任务关联后仍无法找到项目 (itemId: ${fixResult.newItemId})`,
                  );
                }
              } else {
                throw new Error(
                  result.message ||
                    `找不到任务关联的项目且无法修复 (itemId: ${task.itemId}, 名称: ${task.itemTitle || task.name})`,
                );
              }
            } else {
              // 使用重新关联的项目
              const updatedTask = {
                ...task,
                itemId: result.newItemId as string,
              };
              relatedItem = await this.getRelatedItem(updatedTask);

              if (!relatedItem) {
                throw new Error(
                  `重新关联项目失败，仍然找不到有效项目 (itemId: ${result.newItemId})`,
                );
              }
          ...
          if (task.action.seasonNumber > 0 && relatedItem.mediaType === 'tv') {
            if (
              ...
              throw new Error(
                `项目 ${relatedItem.title} 没有第 ${task.action.seasonNumber} 季`,
              );
            }
          }
          ...
          if (!relatedItem.platformUrl) {
            throw new Error(
              `项目 ${relatedItem.title} 没有设置平台URL，无法执行TMDB-Import任务`,
            );
          }
          ...
          // 检查是否需要自动删除已完结项目的任务
          const shouldAutoDelete = await this.checkAndHandleCompletedProject(
            successTask,
            relatedItem,
          );
          ...
          const interruptResult =
            BrowserInterruptDetector.analyzeError(importError);
          const errorMessage =
          ...
          if (interruptResult.isUserInterrupted) {
            ...
            const userMessage =
              BrowserInterruptDetector.generateUserFriendlyMessage(
                interruptResult,
              );
            await taskExecutionLogger.addLog(
              task.id,
            ...
            // 结束执行日志记录
            try {
              await taskExecutionLogger.endTaskExecution(
                task.id,
                false,
                userMessage,
              );
   ...
   * 尝试重新关联任务到有效项目
   */
  private async attemptToRelinkTask(
    task: ScheduledTask,
    ...
    try {
      ...
      if (task.itemTmdbId) {
        console.log(
          `[TaskScheduler] 尝试通过TMDB ID (${task.itemTmdbId}) 匹配项目`,
        );
        ...
        if (matchByTmdbId) {
          console.log(
            `[TaskScheduler] 通过TMDB ID匹配到项目: ${matchByTmdbId.title} (ID: ${matchByTmdbId.id})`,
          );
          return {
            success: true,
      ...
      if (task.itemTitle) {
        console.log(
          `[TaskScheduler] 尝试通过标题 (${task.itemTitle}) 匹配项目`,
        );
        ...
        if (matchByTitle) {
          console.log(
            `[TaskScheduler] 通过标题匹配到项目: ${matchByTitle.title} (ID: ${matchByTitle.id})`,
          );
          return {
            success: true,
      ...
      if (task.name) {
        ...
        console.log(
          `[TaskScheduler] 尝试通过任务名称提取的标题 (${possibleTitle}) 匹配项目`,
        );
        ...
        if (matchByTaskName) {
          console.log(
            `[TaskScheduler] 通过任务名称匹配到项目: ${matchByTaskName.title} (ID: ${matchByTaskName.id})`,
          );
          return {
            success: true,
      ...
      if (sortedItems.length > 0) {
        ...
        console.log(
          `[TaskScheduler] 使用最近创建的项目: ${fallbackItem.title} (ID: ${fallbackItem.id})`,
        );
   ...
   */
  private async executeTMDBImportTask(
    task: ScheduledTask,
    item: TMDBItem,
  ): Promise<void> {
    try {
      ...
      if (!currentItem) {
        ...
        if (!currentItem && item.title) {
          ...
          if (currentItem) {
            console.log(
              `[TaskScheduler] ✓ 通过标题找到项目: ${currentItem.title} (ID: ${currentItem.id})`,
            );
          }
        }
        ...
        if (!currentItem && task.itemTitle) {
          ...
          if (currentItem) {
            console.log(
              `[TaskScheduler] ✓ 通过任务标题找到项目: ${currentItem.title} (ID: ${currentItem.id})`,
            );
          }
        }
        ...
        if (!currentItem) {
          console.log(
            `[TaskScheduler] 可用的项目:`,
            latestItems.map((i) => ({ id: i.id, title: i.title })),
          );
          throw new Error(
            `无法找到项目。尝试的ID: ${item.id}, 任务ID: ${task.itemId}, 标题: ${item.title || task.itemTitle}`,
          );
        }
      }

      console.log(
        `[TaskScheduler] 使用项目数据: ${currentItem.title} (ID: ${currentItem.id})`,
      );

      // 验证基本条件
      if (!currentItem.platformUrl) {
        throw new Error(
          `项目 ${currentItem.title} 缺少平台URL，无法执行TMDB导入`,
        );
      }
      ...
      if (task.action.seasonNumber > 0 && currentItem.mediaType === 'tv') {
        if (
          !currentItem.seasons ||
          !currentItem.seasons.some(
            (s) => s.seasonNumber === task.action.seasonNumber,
          )
        ) {
          throw new Error(
            `项目 ${currentItem.title} 没有第 ${task.action.seasonNumber} 季，请检查季数设置`,
          );
        }
      }
      ...
      try {
        ...
        extractResult = await this.executePlatformExtraction(
          currentItem,
        ...
        try {
          await taskExecutionLogger.addLog(
            task.id,
            '步骤1',
            `播出平台抓取成功，生成CSV: ${extractResult.csvPath}`,
            'success',
          );
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤1',
          `执行过程中发生错误: ${step1Error instanceof Error ? step1Error.message : String(step1Error)}`,
          'error',
        );
        await taskExecutionLogger.completeStep(
          task.id,
          '步骤1',
          false,
          step1Error instanceof Error ? step1Error.message : String(step1Error),
        );
        throw step1Error;
      }
      ...
      try {
        ...
        if (!csvProcessResult.success) {
          await taskExecutionLogger.addLog(
            task.id,
            '步骤2',
            `CSV处理失败: ${csvProcessResult.error}`,
            'error',
          );
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤2',
          `CSV处理成功，删除了 ${csvProcessResult.removedEpisodes?.length || 0} 个已标记集数`,
          'success',
        );
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤2',
          `执行过程中发生错误: ${step2Error instanceof Error ? step2Error.message : String(step2Error)}`,
          'error',
        );
        await taskExecutionLogger.completeStep(
          task.id,
          '步骤2',
          false,
          step2Error instanceof Error ? step2Error.message : String(step2Error),
        );
        throw step2Error;
      }
      ...
      try {
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤2.5',
          `执行过程中发生错误: ${step25Error instanceof Error ? step25Error.message : String(step25Error)}`,
          'error',
        );
        await taskExecutionLogger.completeStep(
          task.id,
      ...
      try {
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤3',
          `调用TMDB导入API，冲突处理: ${conflictAction}`,
          'info',
        );
        ...
        await taskExecutionLogger.addLog(
          task.id,
          '步骤3',
          `执行过程中发生错误: ${step3Error instanceof Error ? step3Error.message : String(step3Error)}`,
          'error',
        );
        await taskExecutionLogger.completeStep(
          task.id,
      ...
      if (!hasItemTitleInCSV) {
        try {
          ...
          if (
            ...
            console.log(
              `[TaskScheduler] CSV中剩余的集数（即成功导入的集数）: [${csvAnalysisResult.remainingEpisodes.join(', ')}]`,
            );
            await taskExecutionLogger.addLog(
              task.id,
              '步骤4',
              `发现 ${csvAnalysisResult.remainingEpisodes.length} 个集数需要标记`,
              'info',
            );
            ...
            if (markingResult) {
              await taskExecutionLogger.addLog(
                task.id,
                '步骤4',
                `成功标记 ${csvAnalysisResult.remainingEpisodes.length} 个集数`,
                'success',
              );
            ...
            if (!csvAnalysisResult.success) {
              await taskExecutionLogger.addLog(
                task.id,
                '步骤4',
                `CSV分析失败: ${csvAnalysisResult.error}`,
                'warning',
              );
            }
          }
          ...
          await taskExecutionLogger.addLog(
            task.id,
            '步骤4',
            `执行过程中发生错误: ${step4Error instanceof Error ? step4Error.message : String(step4Error)}`,
            'error',
          );
          await taskExecutionLogger.completeStep(
            task.id,
  ...
  private async processCSVWithMarkedEpisodes(
    ...
    try {
      ...
      console.log(
        `[TaskScheduler] 已标记的集数: [${markedEpisodes.join(', ')}]`,
      );
      ...
      // API /api/process-csv-episodes 已被删除，直接返回模拟结果
      // 原始API功能已集成到其他系统中

      // 模拟处理逻辑：直接返回CSV路径和标记的集数
      return {
        success: true,
        processedCsvPath: csvPath,
  ...
  private getMarkedEpisodes(item: TMDBItem, seasonNumber: number): number[] {
    ...
    if (item.seasons && item.seasons.length > 0) {
      ...
      if (targetSeason) {
        ...
        console.warn(
          `[TaskScheduler] 未找到第 ${seasonNumber} 季，可用季数: ${item.seasons.map((s) => s.seasonNumber).join(', ')}`,
        );
      }
    ...
    console.log(
      `[TaskScheduler] 最终已标记集数: [${sortedEpisodes.join(', ')}]`,
    );
  ...
  private async checkItemTitleInCSV(
    ...
    try {
      ...
      // API /api/check-csv-title 已被删除，直接返回true（检查失败时默认允许标记）
      // 原始API功能已集成到其他系统中
      return true;
    } catch (error) {
  ...
  private async analyzeCSVRemainingEpisodes(csvPath: string): Promise<{
    ...
    try {
      ...
      // API /api/analyze-csv-episodes 已被删除，直接返回模拟结果
      // 原始API功能已集成到其他系统中

      // 模拟分析结果：返回CSV中的所有episode（未处理的）
      return {
        success: true,
        remainingEpisodes: [], // 假设没有剩余集数需要分析
      };
    } catch (error) {
      // 检查是否是超时错误
      if (error instanceof Error && error.name === 'AbortError') {
  ...
  private async executeTMDBImport(
    ...
    try {
      ...
      const timeoutId = setTimeout(() => {
        console.log(
          `[TaskScheduler] TMDB导入API调用超时 (${timeoutDuration / 60000}分钟)，正在中止请求`,
        );
        controller.abort();
      }, timeoutDuration);
  ...
  private async markEpisodesDirectly(
    ...
    try {
      console.log(
        `[TaskScheduler] 直接标记集数为已完成: 项目="${item.title}", 季=${seasonNumber}, 集数=[${episodeNumbers.join(', ')}]`,
      );
      ...
      if (!targetItem) {
        ...
        if (targetItem) {
          console.log(
            `[TaskScheduler] 通过标题找到项目: ${targetItem.title} (ID: ${targetItem.id})`,
          );
        } else {
          return;
        }
      ...
      if (updateSuccess) {
        console.log(
          `[TaskScheduler] 新标记的集数: [${markedEpisodes.join(', ')}]`,
        );

        // 通知实时同步管理器数据已更新，让主页面能够感知变化
        try {
          const { realtimeSyncManager } =
            await import('@/lib/data/realtime-sync-manager');
          await realtimeSyncManager.notifyDataChange({
  ...
  private async deleteCompletedTask(task: ScheduledTask): Promise<void> {
    try {
      console.log(
        `[TaskScheduler] 删除已完结项目的定时任务: ${task.name} (ID: ${task.id})`,
      );
   ...
   */
  private async markEpisodesAsCompleted(
    item: TMDBItem,
    seasonNumber: number,
    episodeNumbers: number[],
  ): Promise<void> {
    try {
      console.log(
        `[TaskScheduler] 自动标记CSV剩余集数为已完成: 季=${seasonNumber}, 集数=[${episodeNumbers.join(', ')}]`,
      );
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        console.warn(
          `[TaskScheduler] 标记集数失败，但任务继续执行: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
