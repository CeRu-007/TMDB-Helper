...
export function sendToConnection(connectionId: string, data: string): boolean {
  const connection = connections.get(connectionId);
  if (!connection) {
    return false;
  }
...
export function notifyDataChangeFromServer(event: { type: string; data: unknown }): void {
  try {
    const userIds = new Set<string>();
    for (const connection of connections.values()) {
      userIds.add(connection.userId);
