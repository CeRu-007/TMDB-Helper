...
export function AppStateProvider({ children }: { children: React.ReactNode }): JSX.Element {
      ...
      if (savedPreferences) {
        try {
          ...
          if (preferences && typeof preferences === 'object') {
            ...
            logger.debug('StateManager', '用户偏好已加载', preferences);
          }
        } catch (parseError) {
          logger.warn('StateManager', '解析用户偏好数据失败，清除无效数据', parseError);
          localStorage.removeItem('app_preferences');
        }
      }
    } catch (error) {
      logger.warn('StateManager', '加载用户偏好失败', error);
    }
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      logger.warn('StateManager', '保存用户偏好失败', error);
    }
  }, [state.preferences]);
