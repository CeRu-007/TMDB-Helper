...
export class EventDebouncer {
  ...
  constructor(
    private readonly notifyListeners: (eventType: string, event: DataChangeEvent) => void
  ) {}
  ...
  private getEventKey(event: DataChangeEvent): string {
    ...
    switch (event.type) {
      ...
      case 'item_updated':
      case 'item_deleted':
        return (d.id as string) || 'unknown';
      case 'episode_updated':
        return `${(d.item as Record<string, unknown> | undefined)?.id || 'unknown'}_${d.episodeNumber || 'unknown'}`;
      case 'season_added':
      case 'season_deleted':
        return `${(d.item as Record<string, unknown> | undefined)?.id || 'unknown'}_${d.seasonNumber || 'unknown'}`;
      case 'task_completed':
      case 'task_status_changed':
        return (d.taskId as string) || (d.id as string) || 'unknown';
      default:
