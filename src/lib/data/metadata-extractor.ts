...
export class MetadataExtractor {
  ...
  static detectPlatform(url: string): string {
    ...
    if (hostname.includes('hbo.com') || hostname.includes('max.com')) {
      return 'hbo';
    }
    if (hostname.includes('apple.com')) {
      return 'apple';
    }
    if (hostname.includes('paramount.com')) {
      return 'paramount';
    }
  ...
  static buildTMDBUploadUrl(metadata: ExtractedMetadata, tmdbId?: string): string {
    ...
    if (metadata.originalTitle) {
      params.set('original_title', metadata.originalTitle);
    }
    if (metadata.description) {
      params.set('overview', metadata.description);
    }
    if (metadata.year) {
      params.set('release_date', `${metadata.year}-01-01`);
    }
    if (metadata.language) {
      params.set('original_language', metadata.language);
    }
    if (metadata.posterUrl) {
      params.set('poster_url', metadata.posterUrl);
    }
    if (metadata.backdropUrl) {
      params.set('backdrop_url', metadata.backdropUrl);
    }
    if (metadata.trailerUrl) {
      params.set('trailer_url', metadata.trailerUrl);
    }
  ...
  static formatMetadata(metadata: ExtractedMetadata): string {
    ...
    if (metadata.episodeCount) {
      lines.push(`集数: ${metadata.episodeCount}`);
    }
    if (metadata.seasonCount) {
      lines.push(`季数: ${metadata.seasonCount}`);
    }
    if (metadata.network) {
      lines.push(`制作方: ${metadata.network}`);
    }
    if (metadata.language) {
      lines.push(`语言: ${metadata.language}`);
    }
    if (metadata.country) {
      lines.push(`国家: ${metadata.country}`);
    }
    if (metadata.rating) {
      lines.push(`评分: ${metadata.rating}/10`);
    }
    if (metadata.status) {
      lines.push(`状态: ${metadata.status}`);
    }
    if (metadata.airDate) {
      lines.push(`首播: ${metadata.airDate}`);
    }
    if (metadata.endDate) {
      lines.push(`完结: ${metadata.endDate}`);
    }
