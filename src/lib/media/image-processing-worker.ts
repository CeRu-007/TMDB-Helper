...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  try {
    ...
    switch (type) {
      ...
      case 'subtitleScore':
        try {
          const subtitleScore = calculateSubtitleScore(
            imageData,
            width,
            height,
            options?.subtitleDetectionStrength || 0.8
          );
...
function batchAnalyzeImage(
  ...
  try {
    const results: Record<string, number | boolean | Uint8Array | any> = {};

    results.staticScore = calculateStaticScore(imageData, effectiveSampleRate);
    results.subtitleScore = calculateSubtitleScore(
      imageData,
      width,
      height,
      subtitleDetectionStrength
    );
...
function calculateDiversityScore(imageData: ImageData, sampleRate: number = 2): number {
  ...
  const normalizedEntropy = maxPossibleEntropy > 0 ? Math.min(1, entropy / maxPossibleEntropy) : 0;

  // 使用更简单的亮度变化计算替代复杂的边缘计算
  const brightnessVariation = calculateSimplifiedBrightnessVariation(
    imageData,
    effectiveSampleRate
  );

  // 综合评分 (简化公式)
  const diversityScore = normalizedEntropy * 0.7 + brightnessVariation * 0.3;

  return Math.max(0, Math.min(1, diversityScore));
}

// 简化的亮度变化计算，性能更好
function calculateSimplifiedBrightnessVariation(
  imageData: ImageData,
  sampleRate: number = 4
): number {
  const data = imageData.data;
  ...
  for (let i = 0; i < brightnessValues.length; i += sampleStep) {
    const diff = (brightnessValues[i] ?? 0) - avg;
    variance += diff * diff;
    sampleCount++;
  }
...
function calculateStaticScore(imageData: ImageData, sampleRate: number = 1): number {
  ...
  const gridSize = 3;
  const gridEdgeDensity: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
  const gridSamples: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
    ...
    for (let x = 0; x < width; x += sampleRate * 2) {
      ...
      if (x > 0 && y > 0) {
        ...
        if (leftOffset >= 0 && topOffset >= 0) {
          ...
          const topBrightness =
            0.299 * data[topOffset]! + 0.587 * data[topOffset + 1]! + 0.114 * data[topOffset + 2];

          // 如果与左侧或上方像素有明显差异，认为有边缘
          if (
            Math.abs(currentBrightness - leftBrightness) > 20 ||
  ...
  if (gridScores.length > 0) {
    ...
    // 计算中心区域的边缘密度 (中心区域更重要)
    const centerScore = gridSamples[1]![1]! > 0 ? gridEdgeDensity[1]![1]! / gridSamples[1]![1]! : 0;

    // 计算方差 - 内容分布的均匀性
    const variance =
      gridScores.reduce((sum, val) => sum + Math.pow(val - avgScore, 2), 0) / gridScores.length;
...
function calculatePeopleScore(imageData: ImageData, sampleRate: number = 4): number {
  ...
  const gridSize = 3;
  const gridScores: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
  const gridAnimeScores: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
  const gridCounts: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
  ...
  for (let y = 0; y < height; y += sampleRate) {
    for (let x = 0; x < width; x += sampleRate) {
      ...
      const isSkinYCbCr = yColor > 80 && cb > 85 && cb < 135 && cr > 135 && cr < 180;
  ...
  for (let y = 0; y < gridSize; y++) {
    for (let x = 0; x < gridSize; x++) {
      ...
      const animeRegionRatio =
        gridCounts[y]![x]! > 0 ? gridAnimeScores[y]![x]! / gridCounts[y]![x]! : 0;
  ...
  const centerAnimeRatio =
    gridCounts[1]![1]! > 0 ? gridAnimeScores[1]![1]! / gridCounts[1]![1]! : 0;
      ...
      if (!isEdge && y + sampleRate < height) {
        ...
        if (downOffset < data.length) {
          ...
          const downBrightness =
            0.299 * data[downOffset] + 0.587 * data[downOffset + 1] + 0.114 * data[downOffset + 2];
...
// 分析颜色分布
function analyzeColorProfile(
  imageData: ImageData,
  sampleRate: number = 4
): { dominantColors: number[]; colorVariety: number } {
  const data = imageData.data;
  const colorHistogram: { [key: string]: number } = {};
  let totalSamples = 0;
  ...
  // 提取前5个主要颜色的百分比
  const dominantColors = sortedColors.slice(0, 5).map((entry) => entry.percentage);
...
function detectEmptyFrame(imageData: ImageData, sampleRate: number = 2): number {
  ...
  const gridSize = 3;
  const gridContent: number[][] = Array(gridSize)
    .fill(0)
    .map(() => Array(gridSize).fill(0));
  const gridSamples: number[][] = Array(gridSize)
    .fill(0)
  ...
  if (gridContentRatios.length > 0) {
    ...
    if (avg > 0) {
      const variance =
        gridContentRatios.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) /
        gridContentRatios.length;
  ...
  const normalizedBrightnessHistogram = brightnessHistogram.map(
    (count) => count / totalBrightnessSamples
  );
  ...
  for (let y = 1; y < height - 1; y += sampleRate) {
    for (let x = 1; x < width - 1; x += sampleRate) {
      ...
      if (
        leftOffset < 0 ||
        rightOffset >= data.length ||
        topOffset < 0 ||
        bottomOffset >= data.length
      ) {
        continue;
      }
      ...
      // 计算周围像素亮度
      const leftBrightness =
        0.299 * data[leftOffset]! + 0.587 * data[leftOffset + 1]! + 0.114 * data[leftOffset + 2];

      const rightBrightness =
        0.299 * data[rightOffset]! + 0.587 * data[rightOffset + 1]! + 0.114 * data[rightOffset + 2];
      ...
      const laplacian = Math.abs(
        4 * centerBrightness - leftBrightness - rightBrightness - topBrightness - bottomBrightness
      );
...
function calculateAtmosphereScore(imageData: ImageData, sampleRate: number = 4): number {
  ...
  const atmosphereScore =
    colorRichness * 0.3 + contrastScore * 0.25 + compositionScore * 0.25 + visualWeight * 0.2;
