...
export class SRTGenerator {
  ...
  generateSRTFile(ocrResults: OCRResultItem[]): string {
    ...
    return entries.map((entry) => this.formatSRTEntry(entry)).join('\n');
  }
  ...
  private mergeAdjacentResults(results: OCRResultItem[]): OCRResultItem[] {
    ...
    for (const result of results) {
      ...
      if (current) {
        // 检查是否应该合并
        const timeDiff =
          result.timestamp - (current.timestamp + this.estimateDuration(current.text));
  ...
  private estimateDuration(text: string): number {
    ...
    return Math.max(this.config.minDuration, Math.min(this.config.maxDuration, duration));
  }
