...
export class VideoProcessor {
    ...
    try {
      ...
      for (let i = 0; i < sampleTimestamps.length; i++) {
        ...
        try {
          ...
          // 2. 预估合并高度超过限制
          // 3. 最后一帧
          const shouldProcess = extractedFrames.length >= 50 || i === sampleTimestamps.length - 1;

          if (shouldProcess) {
            // 拼接当前批次的图片
            const mergedImage = await this.mergeImagesVertically(
              extractedFrames.map((f) => f.imageUrl)
            );
            ...
            if (mergedImage) {
              ...
              const ocrResults = await this.ocr.recognizeBatch({
                ...
                segments: extractedFrames
                  .map((f) => sampleToSegmentMap.get(f.timestamp))
              ...
              for (let j = 0; j < extractedFrames.length; j++) {
                ...
                if (
                  ocrResults[j]?.text &&
                  !ocrResults[j].text.includes('[无法识别]') &&
                  ocrResults[j].text.trim().length > 0
                ) {
  ...
  private generateSubtitles(
    ...
    for (const frame of validFrames) {
      ...
      const segment = speechSegments.find(
        (s) => frame.timestamp >= s.start && frame.timestamp <= s.end
      );
