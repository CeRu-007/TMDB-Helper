...
async function getStatus(): Promise<NextResponse> {
  try {
    ...
    const hasMainModule =
      exists &&
      (fs.existsSync(path.join(checkPath, 'tmdb_import')) ||
        fs.existsSync(path.join(checkPath, 'tmdb-import')));
...
async function getLatestCommitFallback() {
  try {
    ...
    const messageMatch = html.match(
      /<a class="Link--primary" href="\/[^\/]+\/[^\/]+\/commit\/[^"]+"[^>]*>([^<]+)<\/a>/
    );
 ...
 */
function detectTmdbImportAtDir(dirPath: string): {
  exists: boolean;
  commitSha?: string;
  commitDate?: string;
  commitMessage?: string;
  ...
  const hasMainModule =
    fs.existsSync(path.join(dirPath, 'tmdb_import')) ||
    fs.existsSync(path.join(dirPath, 'tmdb-import'));
...
async function downloadLatest(): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      logger.warn(
        `[TMDB-Import Updater] 直接下载异常: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
async function installUpdate(): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      logger.warn(
        `[TMDB-Import Updater] 保存路径到配置失败: ${error instanceof Error ? error.message : String(error)}`
      );
    }
 ...
 */
function extractTMDBCredentials(configContent: string): {
 ...
 * 将 TMDB 用户凭据写入新的配置文件
 */
function updateConfigWithCredentials(
  configPath: string,
  credentials: { tmdb_username?: string; tmdb_password?: string }
): void {
  if (!fs.existsSync(configPath)) {
    return;
  }
