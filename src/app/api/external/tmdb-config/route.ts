...
async function resolveTmdbPath(pathParam: string | null): Promise<string> {
  const configuredPath = (await ServerConfigManager.getConfigItem('tmdbImportPath')) as
    string | undefined;
  const tmdbDir = pathParam ?? configuredPath ?? path.join(process.cwd(), 'TMDB-Import-master');
