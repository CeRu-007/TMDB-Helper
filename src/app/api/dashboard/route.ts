...
function getProfile(db: any) {
  const totalItems = (
    db.prepare('SELECT COUNT(*) as c FROM items WHERE deletedAt IS NULL').get() as { c: number }
  ).c;
  const completedItems = (
    db
      .prepare('SELECT COUNT(*) as c FROM items WHERE deletedAt IS NULL AND completed = 1')
      .get() as { c: number }
  ).c;
  ...
  const automatedItems = (
    db
      .prepare('SELECT COUNT(DISTINCT itemId) as c FROM schedule_tasks WHERE enabled = 1')
      .get() as { c: number }
  ).c;
  ...
  const firstItemDate = (
    db.prepare('SELECT MIN(createdAt) as d FROM items WHERE deletedAt IS NULL').get() as {
      d: string | null;
    }
  ).d;
  const daysSinceStart = firstItemDate
    ? Math.floor((Date.now() - new Date(firstItemDate).getTime()) / 86400000)
    : 0;
...
function getMilestones(
  db: any
): Array<{ date: string; title: string; description: string; icon: string }> {
  ...
  if (firstCompleted) {
    milestones.push({
      date: firstCompleted.updatedAt,
      title: 'first_completed',
      description: firstCompleted.title,
      icon: 'check-circle',
    });
  }

  const firstAi = db.prepare(`SELECT MIN(createdAt) as date FROM chatHistories`).get() as
    { date: string } | undefined;
  if (firstAi?.date) {
    milestones.push({ date: firstAi.date, title: 'first_ai', description: '', icon: 'bot' });
  }

  const firstSchedule = db.prepare(`SELECT MIN(createdAt) as date FROM schedule_tasks`).get() as
    { date: string } | undefined;
...
function getFunFacts(
  db: any
): Array<{ key: string; value: string | number; label: string; icon: string }> {
  ...
  facts.push({
    key: 'episodes_done',
    value: totalEpisodesDone,
    label: 'episodes_done_desc',
    icon: 'tv',
  });
  ...
  facts.push({
    key: 'automated_items',
    value: automatedItems,
    label: 'automated_items_desc',
    icon: 'bot',
  });
  ...
  const totalCategories = (
    db
      .prepare(
        'SELECT COUNT(DISTINCT category) as c FROM items WHERE deletedAt IS NULL AND category IS NOT NULL'
      )
      .get() as { c: number }
  ).c;
  ...
  facts.push({
    key: 'total_seasons',
    value: totalSeasons,
    label: 'total_seasons_desc',
    icon: 'layers',
  });

  const mostInvestedRow = db
    .prepare(
      `
  ...
  if (mostInvestedRow && mostInvestedRow.done > 0) {
    facts.push({
      key: 'most_invested',
      value: mostInvestedRow.title,
      label: 'most_invested_desc',
      icon: 'clapperboard',
    });
  }
  ...
  if (peakDayRow) {
    ...
    facts.push({
      key: 'peak_day_date',
      value: peakDayRow.day,
      label: 'peak_day_date_desc',
      icon: 'calendar',
    });
  }
  ...
  if (mostEditedRow && mostEditedRow.log_count > 0) {
    facts.push({
      key: 'most_edited',
      value: mostEditedRow.title,
      label: 'most_edited_desc',
      icon: 'pencil',
    });
  }
...
function getAchievements(db: any): Array<{
  id: string;
  icon: string;
  name: string;
  description: string;
  ...
  const hasAiUsage =
    (db.prepare('SELECT COUNT(*) as c FROM chatHistories').get() as { c: number }).c > 0;

  const hasSchedule =
    (
      db.prepare('SELECT COUNT(*) as c FROM schedule_tasks WHERE enabled = 1').get() as {
        c: number;
      }
    ).c > 0;
  const automatedItems = (
  ...
  const nthItemDate = (n: number): string | undefined => {
    const row = db
      .prepare(
        'SELECT createdAt FROM items WHERE deletedAt IS NULL ORDER BY createdAt ASC LIMIT 1 OFFSET ?'
      )
      .get(n - 1) as { createdAt: string } | undefined;
    return row?.createdAt;
  };

  const firstAiDate =
    (db.prepare('SELECT MIN(createdAt) as d FROM chatHistories').get() as { d: string | null }).d ||
    undefined;
  const firstScheduleDate =
    (db.prepare('SELECT MIN(createdAt) as d FROM schedule_tasks').get() as { d: string | null })
      .d || undefined;

  const firstCompletedDate =
    (
      db
        .prepare('SELECT MIN(updatedAt) as d FROM items WHERE deletedAt IS NULL AND completed = 1')
        .get() as { d: string | null }
    ).d || undefined;

  const veteranDate =
    daysSinceStart >= 30
      ? new Date(new Date(firstItemDate!).getTime() + 30 * 86400000).toISOString()
      : undefined;
  const longRunnerDate =
    daysSinceStart >= 180
      ? new Date(new Date(firstItemDate!).getTime() + 180 * 86400000).toISOString()
      : undefined;
  ...
  const allAchievements = [
    {
      ...
      condition: totalItems >= 1,
      unlockedAt: totalItems >= 1 ? firstItemDate || undefined : undefined,
    },
    {
      id: 'fifty_items',
      icon: 'award',
      name: 'fifty_items',
      description: 'fifty_items_desc',
      condition: totalItems >= 50,
      unlockedAt: totalItems >= 50 ? nthItemDate(50) : undefined,
    },
    {
      id: 'hundred_items',
      icon: 'medal',
      name: 'hundred_items',
      description: 'hundred_items_desc',
      condition: totalItems >= 100,
      unlockedAt: totalItems >= 100 ? nthItemDate(100) : undefined,
    },
    {
      id: 'five_hundred',
      icon: 'trophy',
      name: 'five_hundred',
      description: 'five_hundred_desc',
      condition: totalItems >= 500,
      unlockedAt: totalItems >= 500 ? nthItemDate(500) : undefined,
    },
    {
      id: 'lightning',
      icon: 'zap',
      name: 'lightning',
      description: 'lightning_desc',
      condition: maxDailyOps >= 10,
    ...
    },
    {
      ...
      icon: 'bot',
      name: 'ai_pioneer',
      description: 'ai_pioneer_desc',
      condition: hasAiUsage,
      unlockedAt: hasAiUsage ? firstAiDate : undefined,
    },
    {
      id: 'perfectionist',
      icon: 'sparkles',
      name: 'perfectionist',
      description: 'perfectionist_desc',
      condition: completionRate === 100 && totalItems > 0,
      unlockedAt: completionRate === 100 && totalItems > 0 ? firstCompletedDate : undefined,
    },
    {
      id: 'episode_master',
      icon: 'clapperboard',
      name: 'episode_master',
      description: 'episode_master_desc',
      condition: totalEpisodesDone >= 500,
      unlockedAt: undefined,
    },
    {
      id: 'scheduler',
      ...
      unlockedAt: hasSchedule ? firstScheduleDate : undefined,
    },
    {
      id: 'automator',
      icon: 'settings',
      name: 'automator',
      description: 'automator_desc',
      condition: automatedItems >= 5,
      unlockedAt: undefined,
    },
    {
      id: 'veteran',
      ...
      condition: daysSinceStart >= 30,
      unlockedAt: veteranDate,
    },
    {
      id: 'long_runner',
      icon: 'footprints',
      name: 'long_runner',
      description: 'long_runner_desc',
      condition: daysSinceStart >= 180,
      unlockedAt: longRunnerDate,
    },
    {
      id: 'thousand_episodes',
      icon: 'film',
      name: 'thousand_episodes',
      description: 'thousand_episodes_desc',
      condition: totalEpisodesDone >= 1000,
      unlockedAt: undefined,
    },
    {
      id: 'all_categories',
      icon: 'palette',
      name: 'all_categories',
      description: 'all_categories_desc',
...
function getMonthlyComparison(
  db: any
): Array<{ month: string; newItems: number; completedItems: number }> {
  const newRows = db
    .prepare(
      `
...
function getSmartSuggestions(db: any): Array<{ type: string; message: string; priority: number }> {
  ...
  const itemsWithoutCategory = (
    db
      .prepare('SELECT COUNT(*) as c FROM items WHERE deletedAt IS NULL AND category IS NULL')
      .get() as { c: number }
  ).c;
  const failedTasks = (
    db
      .prepare(
        "SELECT COUNT(*) as c FROM schedule_logs WHERE status = 'failed' AND date(startAt) >= date('now', '-7 days')"
      )
      .get() as { c: number }
  ).c;

  const inProgressItems = db
    .prepare(
      `
  ...
  if (failedTasks > 0) {
    suggestions.push({
      type: 'warning',
      message: `近7天有 ${failedTasks} 个定时任务执行失败，请检查配置`,
      priority: 1,
    });
  }
  if (itemsWithoutCategory > 0) {
    suggestions.push({
      type: 'tip',
      message: `${itemsWithoutCategory} 个词条未设置分类，建议补充分类信息`,
      priority: 2,
    });
  }
  if (!hasProgress && totalItems > 0) {
    suggestions.push({
      type: 'tip',
      message: '还没有开始更新维护进度，点击词条即可更新当前集数',
      priority: 3,
    });
  }
  ...
  if (completionRate >= 90) {
    suggestions.push({
      type: 'success',
      message: `维护完成率已达 ${completionRate}%，继续保持！`,
      priority: 5,
    });
  }
...
function getStats(db: any) {
  ...
  const inProgressItems = (
    db
      .prepare('SELECT COUNT(*) as c FROM items WHERE deletedAt IS NULL AND completed = 0')
      .get() as { c: number }
  ).c;
  const today = new Date().toISOString().split('T')[0];
  const todayUpdatedItems = (
    db
      .prepare('SELECT COUNT(*) as c FROM items WHERE deletedAt IS NULL AND date(updatedAt) = ?')
      .get(today) as { c: number }
  ).c;
  const enabledScheduleTasks = (
    db.prepare('SELECT COUNT(*) as c FROM schedule_tasks WHERE enabled = 1').get() as { c: number }
  ).c;
...
function getCompletionProgress(db: any): Array<{
  ...
  `
    ...
    posterUrl: string | null;
    totalEpisodes: number;
    season_total_eps: number;
    season_completed_eps: number;
  }>;
