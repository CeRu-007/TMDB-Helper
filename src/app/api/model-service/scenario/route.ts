...
export async function GET(request: NextRequest) {
  try {
    ...
    // 获取场景配置
    const scenarioConfig = modelServiceConfig.scenarios?.find((s) => s.type === scenario);
    ...
    const models = modelServiceConfig.models.filter((model) => allModelIds.includes(model.id));

    // 获取相关的提供商信息
    const providerIds = [...new Set(models.map((m) => m.providerId))];
    const providers = modelServiceConfig.providers.filter((provider) =>
