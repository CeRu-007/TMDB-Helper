...
export async function POST(request: NextRequest) {
  try {
    ...
    const { modelId, messages, temperature = 0.7, max_tokens = 800 } = body;
    ...
    if (!model) {
      return NextResponse.json({ error: '未找到指定的模型' }, { status: 404 });
    }

    // 查找模型提供商
    const provider = modelServiceConfig.providers.find((p) => p.id === model.providerId);
    if (!provider) {
      return NextResponse.json({ error: '未找到模型的提供商' }, { status: 404 });
    }

    if (!provider.apiKey) {
      return NextResponse.json({ error: '提供商API密钥未配置' }, { status: 400 });
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && parsedError) {
        ...
        if (
          errorCode === 30001 ||
          (errorMessage && errorMessage.includes('account balance is insufficient'))
        ) {
