...
export async function GET(request: NextRequest) {
  try {
    ...
    return createErrorResponse(error instanceof Error ? error.message : '获取配置失败');
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return createErrorResponse(error instanceof Error ? error.message : '保存配置失败');
  }
}
...
const ACTION_HANDLERS: ActionHandlers = {
  'add-provider': (data) => ModelServiceStorage.addProvider(data as ModelProvider),
  ...
  'add-model': (data) => ModelServiceStorage.addModel(data as ModelConfig),
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return createErrorResponse(error instanceof Error ? error.message : '更新配置失败');
  }
}
