...
async function fetchIqiyiTab(): Promise<NextResponse> {
  const url = `${I_QIYI_BASE_URL}/daily_hot_tab`
  ...
  if (!response.ok) {
    throw new Error(`Iqiyi Tab API error: ${response.status}`)
  }
...
export async function GET(): Promise<NextResponse> {
  try {
    return await fetchIqiyiTab()
  } catch (error) {
    console.error('Error fetching Iqiyi tab:', error)
    return NextResponse.json(
      { code: -1, message: 'Failed to fetch Iqiyi tab data', error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
