...
const I_QIYI_BASE_URL = 'https://api.iqiyi.com/3f4/cards.iqiyi.com/views_category/3.0'
...
async function fetchIqiyiContent(date: string): Promise<NextResponse> {
  const url = `${I_QIYI_BASE_URL}/daily_hot_content`
  ...
  if (!response.ok) {
    throw new Error(`Iqiyi Content API error: ${response.status}`)
  }
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!date) {
    return NextResponse.json(
      { code: -1, message: 'Missing required parameter: date' },
      { status: 400 }
    )
  }

  try {
    return await fetchIqiyiContent(date)
  } catch (error) {
    console.error('Error fetching Iqiyi content:', error)
    return NextResponse.json(
      { code: -1, message: 'Failed to fetch Iqiyi content data', error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
