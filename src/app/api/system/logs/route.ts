...
function listLogFiles(): LogFileEntry[] {
  try {
    ...
    files.sort((a, b) => {
      if (a.name === 'app.log') return -1
      if (b.name === 'app.log') return 1
...
function readLogFile(
  ...
  if (!resolvedPath.startsWith(normalizedLogDir)) return null
...
function deleteLogFile(filename?: string): boolean {
  try {
    ...
    if (filename) {
      const resolvedPath = path.resolve(LOG_DIR, filename)
      if (!resolvedPath.startsWith(path.resolve(LOG_DIR))) return false
    ...
    for (const entry of entries) {
      if (entry.endsWith('.log') || /\.\d+\.log$/.test(entry)) {
        try { fs.unlinkSync(path.join(LOG_DIR, entry)) } catch {}
      }
    }
    return true
  } catch {
