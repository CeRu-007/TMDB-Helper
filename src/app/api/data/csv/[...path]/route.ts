...
async function handleReadCsv(request: NextRequest) {
  try {
    ...
    if (!workingDirectory) {
      return NextResponse.json({ success: false, error: '缺少工作目录参数' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: '读取CSV文件失败' }, { status: 500 });
  }
}
...
async function handleSaveCsv(request: NextRequest) {
  try {
    ...
    if (!filePath) {
      return NextResponse.json({ success: false, error: '缺少文件路径参数' }, { status: 400 });
    }
    ...
    if (content) {
      ...
      // 传递的结构化数据，需要序列化
      const { serializeCsvData } = await import('@/lib/data/csv-processor-client');
    ...
    return NextResponse.json({ success: false, error: '保存CSV文件失败' }, { status: 500 });
  }
}
...
async function handleVerifyCsv(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: '验证CSV文件失败' }, { status: 500 });
  }
}
...
async function handleDeleteCsv(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: '删除CSV文件失败' }, { status: 500 });
  }
}
