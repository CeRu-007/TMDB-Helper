  ...
  try {
    ...
    if (!description && (!keywords || keywords.length === 0)) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: { code: 'VALIDATION_ERROR', message: '描述或关键词未提供', timestamp: Date.now() },
          data: null,
          timestamp: Date.now(),
        },
        { status: 400 }
    ...
    const movies = rankedResults.map((result) => ({
      ...
      originalTitle:
        ((result as unknown as Record<string, unknown>).original_title as string) ||
        ((result as unknown as Record<string, unknown>).original_name as string),
    ...
    >({
      success: true,
      data: {
        movies: movies.slice(0, 20) as unknown as SearchResult[], // 限制返回结果数量
      },
      timestamp: Date.now(),
    });
  } catch (error: unknown) {
...
async function searchTMDB(type: 'movie' | 'tv', query: string, apiKey: string) {
  ...
  return (
    data.results?.map((item: TMDBResult) => ({
      ...item,
      media_type: type,
      searchQuery: query,
    })) || []
...
function rankResults(
  ...
  return results
    .map((result) => {
      ...
      matchScore +=
        Math.log10(
          (((result as unknown as Record<string, unknown>).popularity as number) || 1) + 1
        ) * 10;
      ...
      const originalTitle = (
        (resultAny.original_title as string) ||
        (resultAny.original_name as string) ||
        ''
      ).toLowerCase();
