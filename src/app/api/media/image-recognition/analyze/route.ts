  ...
  try {
    ...
    if (!image) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: { code: 'VALIDATION_ERROR', message: '图片数据未提供', timestamp: Date.now() },
          data: null,
          timestamp: Date.now(),
        },
        { status: 400 }
    ...
    // 从模型服务系统获取图像分析场景配置
    const modelServiceResponse = await fetch(
      `${process.env.NEXTAUTH_URL || 'http://localhost:4949'}/api/model-service/scenario?scenario=image_analysis`
    );
    if (!modelServiceResponse.ok) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: { code: 'SERVICE_ERROR', message: '获取模型服务配置失败', timestamp: Date.now() },
          data: null,
          timestamp: Date.now(),
        },
        { status: 500 }
    ...
    if (
      !modelServiceData.success ||
      !modelServiceData.scenario ||
      !modelServiceData.scenario.primaryModelId
    ) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: {
            code: 'CONFIG_ERROR',
            message: '图像分析场景未配置，请在模型服务-使用场景中配置模型',
            timestamp: Date.now(),
          },
          data: null,
          timestamp: Date.now(),
    ...
    const provider = modelServiceData.providers.find(
      (p: ModelProvider) => p.id === primaryModel.providerId
    );
    ...
    // 获取场景参数
    const scenarioParams = (modelServiceData.scenario as Record<string, unknown>).parameters as
      Record<string, unknown> | undefined;
    const temperature = (scenarioParams?.temperature as number) || 0.7;
    const maxTokens = (scenarioParams?.max_tokens as number) || 2000;
...
请尽可能详细和准确地描述，这将用于在影视数据库中搜索匹配的作品。`,
      ...
      return NextResponse.json<ApiResponse<null>>(
        {
          ...
          error: {
            code: 'API_ERROR',
            message: `魔搭社区API调用失败: ${response.status} ${response.statusText}`,
            timestamp: Date.now(),
    ...
    if (!content) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: { code: 'EMPTY_RESPONSE', message: '模型未返回有效内容', timestamp: Date.now() },
          data: null,
          timestamp: Date.now(),
        },
        { status: 500 }
