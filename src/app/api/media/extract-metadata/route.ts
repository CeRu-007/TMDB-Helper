  ...
  try {
    ...
    if (!url) {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          error: { code: 'VALIDATION_ERROR', message: 'URL is required', timestamp: Date.now() },
          data: null,
          timestamp: Date.now(),
        },
        { status: 400 }
    ...
    return NextResponse.json<ApiResponse<null>>(
      {
        success: false,
        error: { code: 'INTERNAL_ERROR', message: errorMessage, timestamp: Date.now() },
        data: null,
        timestamp: Date.now(),
      },
      { status: 500 }
...
async function extractMetadataFromUrl(
  ...
  try {
    ...
    // 使用平台特定的解析器
    let metadata: ExtractedMetadata = { title: '' };
...
function parseGenericMetadata(html: string, platform: string, url: string): ExtractedMetadata {
  const metadata: ExtractedMetadata = { title: '' };
      ...
      try {
        ...
        if (
          ...
          if (data.numberOfEpisodes) {
            metadata.episodeCount = data.numberOfEpisodes;
          }
          if (data.numberOfSeasons) {
            metadata.seasonCount = data.numberOfSeasons;
          }
          ...
          if (data.image) {
            ...
            if (!metadata.posterUrl) {
              metadata.posterUrl = resolveUrl(imageUrl, url);
            }
          }
        }
      } catch (e) {
  ...
  if (!metadata.year) {
    const yearMatch =
      html.match(/(?:年份|Year|发布|Release)[^>]*>.*?(\d{4})/i) ||
      html.match(/(\d{4})\s*年/) ||
      html.match(/20\d{2}/);
    if (yearMatch) {
...
function cleanMetadata(metadata: ExtractedMetadata): ExtractedMetadata {
  ...
  if (cleaned.director) {
    ...
    if (cleaned.director.length === 0) {
      delete cleaned.director;
    }
  }

  if (cleaned.cast) {
    cleaned.cast = cleaned.cast.filter((c) => c && c.trim()).map((c) => c.trim());
    if (cleaned.cast.length === 0) {
      delete cleaned.cast;
    }
  }
