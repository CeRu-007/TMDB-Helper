...
export const GET = AuthMiddleware.withAuth(async (request: NextRequest) => {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ success: false, error: '未找到用户ID' }, { status: 400 });
    }
...
export const POST = AuthMiddleware.withAuth(async (request: NextRequest) => {
  try {
    ...
    if (requestUserId && requestUserId !== userId) {
      return NextResponse.json({ success: false, error: '用户ID不匹配' }, { status: 403 });
    }

    logger.info(
      `在服务器端接收到用户信息更新请求: displayName=${displayName}, avatarUrl=${avatarUrl}, usageTime=${body.usageTime}`
    );
