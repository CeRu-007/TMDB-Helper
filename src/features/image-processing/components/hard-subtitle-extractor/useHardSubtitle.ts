...
interface UseHardSubtitleReturn {
  ...
  setVideoElementRef: (video: HTMLVideoElement | null) => void;
}
...
export function useHardSubtitle(): UseHardSubtitleReturn {
  ...
  const saveConfig = useCallback(
    (newConfig: Config | ((prev: Config) => Config)) => {
  ...
  const updateSubtitleRegion = useCallback((id: string, region: Partial<BoundingBox>) => {
    setSubtitleRegions((prev) => prev.map((r) => (r.id === id ? { ...r, ...region } : r)));
  }, []);
      ...
      const result = await processorRef.current.process(
        ...
        (progressInfo: ProcessProgress) => {
          ...
          if (progressInfo.frameResult) {
            subtitleIndex++;
            const { timestamp, text, confidence, segmentStart, segmentEnd } =
              progressInfo.frameResult;
  ...
  }, [config, subtitleRegions, videoFile, videoUrl]);
