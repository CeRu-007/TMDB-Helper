...
export function VideoPreview({
  ...
  // 获取相对于视频的坐标
  const getVideoRelativePos = useCallback(
  ...
  // 检查是否点击了删除按钮
  const isInDeleteButton = useCallback(
    (x: number, y: number, region: BoundingBox, video: HTMLVideoElement): boolean => {
  ...
  const getRegionAtPoint = useCallback(
    (x: number, y: number, video: HTMLVideoElement): BoundingBox | null => {
    ...
    (e: React.MouseEvent) => {
      ...
      if (isMoving && selectedRegionId) {
        const distance = Math.sqrt(
          Math.pow(coords.x - moveOffset.x, 2) + Math.pow(coords.y - moveOffset.y, 2)
        );
        if (distance > 3) {
  ...
  if (!videoUrl) {
    return (
      <div ref={wrapperRef} className="w-full h-full flex items-center justify-center bg-gray-900">
        <div className="text-center">
          <p className="text-gray-400 mb-2">{t('hardSubtitle.uploadVideoOrUrl')}</p>
          <p className="text-gray-500 text-sm">{t('hardSubtitle.dragToSelectRegion')}</p>
        </div>
      </div>
    );
  }
