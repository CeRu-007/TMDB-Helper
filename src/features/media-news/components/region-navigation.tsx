  ...
  const getValidItems = (regionItems: any[]) => {
    return regionItems.filter(
      (item) =>
        !items.some(
          (existingItem: any) =>
            existingItem.tmdbId === item.id.toString() && existingItem.mediaType === item.mediaType
        )
    );
  };
              ...
              >
                <div className="p-2 max-h-[320px] overflow-y-auto scrollbar-thin">
                  {REGION_GROUPS.map((group) => {
                    const groupKey =
                      group.name === '亚洲' ? 'regionGroup.asia' : 'regionGroup.western';
                    return (
                          ...
                          {group.regions.map((regionId) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {validItems.length > 0 && (
                                  <span
                                    className={`px-1.5 py-0.5 text-xs rounded-full font-medium min-w-[1.25rem] text-center ${
                                      isActive
