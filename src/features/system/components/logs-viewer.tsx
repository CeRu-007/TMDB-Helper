...
export function LogsViewer() {
  ...
  const displayedLines = useMemo(() => {
    ...
    const allLines = liveLines.length > 0 ? [...baseLines, ...liveLines] : baseLines;

    return allLines.filter((l) => {
      if (!matchesLevel(l, levelFilter)) {
        return false;
      }
      if (!matchesSearch(l, searchQuery)) {
        return false;
      }
      return true;
    });
    ...
    <div className="min-h-[calc(100vh-4rem)] flex flex-col md:flex-row">
      <div className="w-full md:w-64 border-b md:border-r bg-muted/50 flex flex-col max-h-40 md:max-h-none">
        <div className="p-4 border-b dark:border-border">
          ...
          {files.length > 0 && <p className="text-xs text-gray-400">{files.length} 个文件</p>}
        </div>
          ...
          <div className="p-2 space-y-1">
            {files.length === 0 ? (
              ...
              files.map((f) => (
                ...
                >
                  <FileText
                    className={`h-4 w-4 mt-0.5 flex-shrink-0 ${selectedFile === f.name ? 'text-blue-500' : 'text-gray-400'}`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm truncate">{f.name}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{formatFileSize(f.size)}</div>
                  </div>
                  <ChevronRight
                    className={`h-3.5 w-3.5 mt-1 flex-shrink-0 transition-transform ${
