...
export default function DependencyInstaller() {
  ...
  const installPythonPackages = async () => {
    ...
    try {
      const packagesToInstall = PYTHON_PACKAGES.filter((pkg) => !status.packages[pkg.name]).map(
        (pkg) => pkg.name
      );
      ...
      if (data.success) {
        ...
        if (!hasErrors) {
          toast({
            ...
            description: t('dependencyInstaller.successInstallCount', {
              count: data.data.summary.success,
            }),
          });
          // 重新检查状态
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-6">{t('dependencyInstaller.description')}</p>
      </div>
      ...
      {status?.environment && (
        <div className={`p-3 ${envDisplay.bgColor} rounded-lg border ${envDisplay.borderColor}`}>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium">运行环境: {envDisplay.label}</span>
            {status.platform && <span className="text-xs text-gray-500">({status.platform})</span>}
          </div>
        </div>
      )}
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex items-center justify-between">
                ...
                <div>
                  <div className="font-medium">
                    {status.python.available
                      ? t('dependencyInstaller.pythonInstalled')
                      : t('dependencyInstaller.pythonNotInstalled')}
                  </div>
              ...
              <Button variant="outline" size="sm" onClick={checkDependencies} disabled={loading}>
          ...
          {PYTHON_PACKAGES.map((pkg) => (
            <div key={pkg.name} className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-medium">
                    {t(`dependencyInstaller.${pkg.translationKey}`)}
                  </div>
                  <div className="text-sm text-gray-500">
                    {t(`dependencyInstaller.${pkg.translationKey}Note`)}
                  </div>
                </div>
              </div>
              <div className="flex items-center space-x-2">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm text-blue-800 dark:text-blue-200 font-medium">
                  {t('dependencyInstaller.pythonDepsNote')}
                </p>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm text-amber-800 dark:text-amber-200 font-medium">
                  {t('dependencyInstaller.browserNote')}
                </p>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-sm text-red-800 dark:text-red-200 font-medium">
                    {t('dependencyInstaller.prerequisite')}
                  </p>
