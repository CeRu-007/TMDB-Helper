...
const GRADIENT_PRESETS = [
  { name: "Sunset", value: "linear-gradient(135deg, hsl(15 20% 10%) 0%, hsl(350 20% 12%) 50%, hsl(25 25% 8%) 100%)" },
...
export const ThemeCustomizer = memo(function ThemeCustomizer({
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <RadioGroup
              ...
              onValueChange={(v) =>
                onUpdateTheme({ typography: { ...currentTheme.typography, fontSize: v as "small" | "medium" | "large" } })
              }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Select
              value={currentTheme.background.type === "image" ? "gradient" : currentTheme.background.type}
              onValueChange={(v) => {
                onUpdateTheme({ background: { ...currentTheme.background, type: v as "solid" | "gradient" } })
              }}
            >
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {GRADIENT_PRESETS.map((preset) => (
                  <button
                    key={preset.name}
                    onClick={() => {
                      onUpdateTheme({ background: { ...currentTheme.background, type: "gradient", value: preset.value } })
                    }}
          ...
          <div>
            <Label className="text-sm font-medium">{t("bgOpacity")}: {Math.round(currentTheme.background.opacity * 100)}%</Label>
            <Slider
              value={[currentTheme.background.opacity * 100]}
              onValueChange={([v]) => {
                if (v !== undefined) onUpdateTheme({ background: { ...currentTheme.background, opacity: v / 100 } })
              }}
          ...
          <div>
            <Label className="text-sm font-medium">{t("bgBlur")}: {currentTheme.background.blur}px</Label>
            <Slider
              value={[currentTheme.background.blur]}
              onValueChange={([v]) => {
                if (v !== undefined) onUpdateTheme({ background: { ...currentTheme.background, blur: v } })
              }}
              max={20} step={1} className="mt-2"
            />
          </div>
        </CardContent>
      </Card>
      ...
      {/* Reset button */}
      {isCustomized && (
        <button onClick={onResetTheme} className="text-sm text-muted-foreground hover:text-foreground underline">
          {t("resetToPreset")}
        </button>
      )}
    </div>
