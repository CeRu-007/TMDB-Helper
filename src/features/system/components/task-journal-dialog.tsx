...
export function TaskJournalDialog({ open, onOpenChange }: TaskJournalDialogProps) {
  ...
  }, [statusFilter, setJournalUnreadCount]);
              ...
              {loading ? (
                ...
                <div className="space-y-2 pr-3">
                  {Object.entries(groupedJournals).map(([itemId, group]) => {
                    ...
                    const groupSuccessCount = group.entries.filter(
                      (e) => e.status === 'success'
                    ).length;
                    const groupFailedCount = group.entries.filter(
                      (e) => e.status === 'failed'
                    ).length;
                      ...
                      <div key={itemId} className="group rounded-lg border bg-card">
                        <div className="w-full flex items-center gap-2 px-4 py-3 hover:bg-muted/30 transition-colors text-left">
                          ...
                          >
                            ...
                            <span className="font-medium text-sm truncate">{group.itemTitle}</span>
                          </button>
                          {hasUnread && (
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            ...
                            {groupFailedCount > 0 && (
                              <Badge
                                variant="secondary"
                                className="text-[10px] h-4 px-1.5 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
                              >
                                {groupFailedCount}
                              </Badge>
                            )}
                            <span className="text-[10px]">
                              {t('totalCount', { count: group.entries.length })}
                            </span>
                          </div>
                          <Button
                            variant="ghost"
                        ...
                        {isExpanded && (
                          <div className="border-t">
                            {group.entries.map((entry) => (
                              <JournalEntryItem key={entry.id} entry={entry} t={t} />
                            ))}
                          </div>
                        )}
                      </div>
...
function JournalEntryItem({ entry, t }: { entry: TaskJournal; t: TFunction<'journal'> }) {
  ...
  return (
    <div
      className={`px-4 py-3 border-b last:border-b-0 ${!entry.read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
            ...
            {entry.endAt && <span>{formatDuration(entry.startAt, entry.endAt)}</span>}
            {entry.tmdbUrl && (
              <a
                href={entry.tmdbUrl}
                target="_blank"
              ...
              {showDetails && (
                <div className="mt-1 text-xs text-muted-foreground bg-muted/50 rounded px-2 py-1.5 space-y-0.5">
                  {dataPreview.episodeCount !== undefined && (
                    <div>
                      {t('episodeCount')}: {dataPreview.episodeCount}
                    </div>
                  )}
                  {dataPreview.rawEpisodeCount !== undefined && (
                    <div>
                      {t('rawEpisodeCount')}: {dataPreview.rawEpisodeCount}
                    </div>
                  )}
                  {dataPreview.incompleteEpisodes && dataPreview.incompleteEpisodes.length > 0 && (
                    <div className="text-amber-600 dark:text-amber-400">
                      {t('incompleteEpisodes')}: {dataPreview.incompleteEpisodes.join(', ')}
                    </div>
                  )}
                  {dataPreview.csvLength !== undefined && (
                    <div>
                      {t('csvLength')}: {dataPreview.csvLength}
                    </div>
                  )}
                  {dataPreview.cleanedLength !== undefined && (
                    <div>
                      {t('cleanedLength')}: {dataPreview.cleanedLength}
                    </div>
                  )}
                  {dataPreview.autoImport !== undefined && (
                    <div>
                      {t('autoImport')}: {dataPreview.autoImport ? t('yes') : t('no')}
                    </div>
                  )}
                </div>
              )}
