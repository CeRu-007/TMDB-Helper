...
export type CategoryType = 'all' | 'anime' | 'domestic' | 'following';
        ...
        <div className="flex items-center justify-between max-sm:flex-wrap max-sm:gap-2 px-6 max-sm:px-3 py-3.5 max-sm:py-2.5 bg-card border-b border-border">
          <div className="flex items-center gap-3 max-sm:gap-1.5">
            <div className="flex items-center gap-2 max-sm:gap-1.5">
              ...
              <span className="text-base max-sm:text-sm font-medium text-foreground">
                {t('thisWeekAnime')}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1 bg-muted rounded-lg p-1 max-sm:p-0.5">
              <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7 max-sm:h-6 max-sm:w-6 hover:bg-white dark:hover:bg-accent"
              >
                <ChevronLeft className="h-4 w-4 max-sm:h-3.5 max-sm:w-3.5" />
              </Button>
              <span className="px-2 max-sm:px-1 text-sm max-sm:text-xs text-foreground min-w-[80px] max-sm:min-w-[60px] text-center font-medium">
                {t('monthYear', { year: dateInfo.year, month: dateInfo.month })}
              </span>
              <Button
                variant="ghost"
                size="icon"
          ...
          <div className="flex items-center gap-2.5 max-sm:gap-1.5 max-sm:w-full max-sm:overflow-x-auto max-sm:flex-nowrap max-sm:scrollbar-hide">
            <div className="flex items-center bg-muted rounded-lg p-1 max-sm:p-0.5 max-sm:flex-shrink-0">
              {CATEGORIES.map((cat) => (
                ...
                >
                  <span
                    className={cn('w-2 h-2 rounded-full max-sm:w-1.5 max-sm:h-1.5', cat.color)}
                  />
                  {cat.label}
                </button>
              ))}
            ...
            <Button
              ...
              disabled={loading}
              className="border-border text-sm max-sm:text-xs font-medium max-sm:h-8"
            >
              <RefreshCw
                className={cn(
                  'h-4 w-4 mr-1.5 max-sm:mr-1 max-sm:h-3.5 max-sm:w-3.5',
