...
class IqiyiScheduleAdapter implements PlatformScheduleAdapter {
  ...
  private async fetchAllContent(
    dateTabs: IqiyiDateTab[],
    year: number
  ): Promise<(IqiyiContentResponse | null)[]> {
  ...
  private async fetchContentByDate(
    dateStr: string,
    year: number
  ): Promise<IqiyiContentResponse | null> {
  ...
  private transformDay(
    tab: IqiyiDateTab,
    contentData: IqiyiContentResponse | null,
    year: number
  ): ScheduleDay | null {
    if (!tab?.date) {
      return null;
  ...
  private transformEpisode(
    block: IqiyiContentResponse['cards'][0]['blocks'][0]
    ...
    const albumId =
      block.actions?.click_event?.data?.album_id ||
    ...
    const pubIndexMetaNames = [
      META_FIELD_NAMES.SUBTITLE,
      META_FIELD_NAMES.EPISODE,
      META_FIELD_NAMES.UPDATE,
      META_FIELD_NAMES.PUB_INDEX,
    ];
    const pubIndex = this.findMetaValue(metas, pubIndexMetaNames) || '更新中';

    const pubTimeMetaNames = [
      META_FIELD_NAMES.TIME,
      META_FIELD_NAMES.PUB_TIME,
      META_FIELD_NAMES.PUBLISH_TIME,
    ];
  ...
  private extractCoverImage(images: Array<{ url: string; name: string }>): string {
    let cover =
      images.find((img) => img.name === 'poster')?.url ||
  ...
  private extractMetaValue(
    metas: Array<{ text: string; name: string }>,
    name: string
  ): string | undefined {
    return metas.find((meta) => meta.name === name)?.text;
  }

  private findMetaValue(
    metas: Array<{ text: string; name: string }>,
    names: string[]
  ): string | undefined {
    return names.reduce<string | undefined>(
      (result, name) => result || this.extractMetaValue(metas, name),
      undefined
    );
  }
