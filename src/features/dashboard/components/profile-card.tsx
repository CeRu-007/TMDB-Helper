    ...
    <div className="relative overflow-hidden rounded-2xl border border-border bg-muted/50">
      {hasPosters ? (
        <>
          ...
          <div
            className={`absolute inset-0 bg-gradient-to-br ${style.gradient} opacity-20 mix-blend-overlay`}
          />
        ...
        <div
          className={`absolute top-0 left-0 right-0 h-32 bg-gradient-to-r ${style.gradient} opacity-10`}
        />
      )}
      ...
      <div className="relative p-4 md:p-6">
        <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6">
          <div className="relative flex-shrink-0">
            ...
            <div
              className={`relative w-20 h-20 rounded-full bg-gradient-to-br ${style.gradient} flex items-center justify-center shadow-lg overflow-hidden ring-4 ring-white/30 dark:ring-border/50`}
            >
              {userInfo?.avatarUrl ? (
                <UserAvatarImage
            ...
            <div
              className={`absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-gradient-to-br ${style.gradient} shadow-md flex items-center justify-center border border-white/30 dark:border-border`}
            >
          ...
          <div className="flex-1 min-w-0">
            <div className="h-5" />
            <h2
              className={`text-xl font-bold mb-2 drop-shadow-sm ${hasPosters ? 'text-white' : 'text-foreground'}`}
            >
              {t(`profile.types.${profile.type}.name`)}
            </h2>
            <p
              className={`text-sm drop-shadow-sm ${hasPosters ? 'text-white/90' : 'text-muted-foreground'}`}
            >
        ...
        <div className={`mt-4 md:mt-6 grid grid-cols-2 sm:grid-cols-5 gap-4`}>
          <div className="text-center">
            <p
              className={`text-2xl font-bold drop-shadow-sm ${hasPosters ? 'text-white' : 'text-foreground'}`}
            >
              {profile.totalItems}
            </p>
            <p className={`text-xs ${hasPosters ? 'text-white/70' : 'text-muted-foreground'}`}>
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-blue-400 drop-shadow-sm">
              {profile.automationRate}%
            </p>
            <p className={`text-xs ${hasPosters ? 'text-white/70' : 'text-muted-foreground'}`}>
              {t('profile.automationRate')}
            </p>
          </div>
          <div className="text-center">
            <p
          ...
          <div className="text-center">
            ...
            >
              {profile.peakWeekday !== null
                ? t(
                    `weekdays.${['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'][profile.peakWeekday]}`
                  )
                : '-'}
            </p>
            <p className={`text-xs ${hasPosters ? 'text-white/70' : 'text-muted-foreground'}`}>
              {t('profile.peakWeekday')}
            </p>
          </div>
        </div>
        ...
        {profile.firstItemTitle && (
          <div className={`mt-4`}>
            <p className={`text-xs ${hasPosters ? 'text-white/60' : 'text-muted-foreground'}`}>
              {t('profile.firstItem', {
                title: profile.firstItemTitle,
                date: profile.firstItemDate
                  ? new Date(profile.firstItemDate).toLocaleDateString()
                  : '',
              })}
            </p>
          </div>
