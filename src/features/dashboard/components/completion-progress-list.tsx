  ...
  return (
    <div className="rounded-xl border border-border bg-muted/50 p-4 md:p-5">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2 md:space-y-3 max-h-[320px] overflow-y-auto pr-1">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-foreground truncate pr-2">{item.title}</span>
                  <span
                    className={`text-xs font-medium flex-shrink-0 ${getProgressColor(item.progress)}`}
                  >
                ...
                <Progress
                  value={item.progress}
                  className={`h-1.5 ${getProgressBg(item.progress)}`}
                />
              </div>
            </div>
          ))}
