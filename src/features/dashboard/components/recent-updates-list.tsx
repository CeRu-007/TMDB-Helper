...
export function RecentUpdatesList({ items }: RecentUpdatesListProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/50 p-4 md:p-5">
      <h3 className="text-sm font-medium text-foreground mb-3 md:mb-4">{t('recentUpdates')}</h3>
      {items.length === 0 ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {item.category && (
                    <Badge
                      variant="secondary"
                      className={`text-[10px] px-1.5 py-0 ${CATEGORY_STYLES[item.category] || 'bg-muted text-muted-foreground'}`}
                    >
