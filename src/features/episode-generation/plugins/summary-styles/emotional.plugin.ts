...
export const emotionalPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'emotional',
      type: PluginType.SummaryStyle,
      name: '情感',
      description: '注重情感表达和共鸣',
      icon: '💝',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['emotional', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写情感风格的简介。

任务：为第 ${content.episodeNumber} 集撰写情感风格的简介
