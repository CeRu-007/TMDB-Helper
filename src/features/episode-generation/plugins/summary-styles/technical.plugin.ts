...
export const technicalPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'technical',
      type: PluginType.SummaryStyle,
      name: '技术分析',
      description: '从制作技术角度进行描述',
      icon: '🎯',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['technical', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写技术分析风格的简介。

任务：为第 ${content.episodeNumber} 集撰写技术分析风格的简介
