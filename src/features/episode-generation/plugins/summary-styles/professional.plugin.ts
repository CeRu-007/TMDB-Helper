...
export const professionalPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'professional',
      type: PluginType.SummaryStyle,
      name: '专业',
      description: '正式、准确的描述风格',
      icon: '📝',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['professional', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写专业风格的简介。

任务：为第 ${content.episodeNumber} 集撰写专业风格的简介
