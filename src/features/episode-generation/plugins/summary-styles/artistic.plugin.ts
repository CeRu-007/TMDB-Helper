...
export const artisticPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'artistic',
      type: PluginType.SummaryStyle,
      name: '文艺风格',
      description: '优雅文艺的表达方式',
      icon: '🎨',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['artistic', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写文艺风格风格的简介。

任务：为第 ${content.episodeNumber} 集撰写文艺风格风格的简介
