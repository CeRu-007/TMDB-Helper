...
export const detailedPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'detailed',
      type: PluginType.SummaryStyle,
      name: '详细描述',
      description: '丰富详尽的内容介绍',
      icon: '📖',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['detailed', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写详细描述风格的简介。

任务：为第 ${content.episodeNumber} 集撰写详细描述风格的简介
