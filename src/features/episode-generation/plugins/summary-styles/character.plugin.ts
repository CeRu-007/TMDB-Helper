...
export const characterPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'character',
      type: PluginType.SummaryStyle,
      name: '角色聚焦',
      description: '以角色发展和关系为中心',
      icon: '👥',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['character', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写角色聚焦风格的简介。

任务：为第 ${content.episodeNumber} 集撰写角色聚焦风格的简介
