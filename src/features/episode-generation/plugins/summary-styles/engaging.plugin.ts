...
export const engagingPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'engaging',
      type: PluginType.SummaryStyle,
      name: '引人入胜',
      description: '吸引观众的生动描述',
      icon: '✨',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['engaging', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写引人入胜风格的简介。

任务：为第 ${content.episodeNumber} 集撰写引人入胜风格的简介
