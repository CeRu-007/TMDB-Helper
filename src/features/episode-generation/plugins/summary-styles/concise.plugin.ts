...
export const concisePlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'concise',
      type: PluginType.SummaryStyle,
      name: '简洁明了',
      description: '简短直接的核心内容描述',
      icon: '📋',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['concise', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写简洁明了风格的简介。

任务：为第 ${content.episodeNumber} 集撰写简洁明了风格的简介
