...
export const suspensefulPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'suspenseful',
      type: PluginType.SummaryStyle,
      name: '悬疑',
      description: '营造紧张悬疑氛围',
      icon: '🔍',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['suspenseful', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写悬疑风格的简介。

任务：为第 ${content.episodeNumber} 集撰写悬疑风格的简介
