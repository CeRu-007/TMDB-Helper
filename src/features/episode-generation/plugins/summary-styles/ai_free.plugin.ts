...
export const ai_freePlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'ai_free',
      type: PluginType.SummaryStyle,
      name: 'AI自由发挥',
      description: 'AI根据内容自由生成',
      icon: '🤖',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['ai_free', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写AI自由发挥风格的简介。

任务：为第 ${content.episodeNumber} 集撰写AI自由发挥风格的简介
