...
export const accessiblePlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'accessible',
      type: PluginType.SummaryStyle,
      name: '通俗易懂',
      description: '大众化的表达方式',
      icon: '👨‍👩‍👧‍👦',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['accessible', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写通俗易懂风格的简介。

任务：为第 ${content.episodeNumber} 集撰写通俗易懂风格的简介
