...
export const plotPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'plot',
      type: PluginType.SummaryStyle,
      name: '情节推进',
      description: '强调故事情节的发展脉络',
      icon: '🧩',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写情节推进风格的简介。

任务：为第 ${content.episodeNumber} 集撰写情节推进风格的简介
