...
export const humorousPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'humorous',
      type: PluginType.SummaryStyle,
      name: '幽默',
      description: '轻松幽默的表达方式',
      icon: '😄',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['humorous', 'builtin'],
      metadata: {
        category: 'summary',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写幽默风格的简介。

任务：为第 ${content.episodeNumber} 集撰写幽默风格的简介
