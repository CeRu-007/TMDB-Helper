...
export const actionPlugin: ISummaryStylePlugin = new (class extends BasePlugin implements ISummaryStylePlugin {
  constructor() {
    super({
      id: 'action',
      type: PluginType.SummaryStyle,
      name: '动作导向',
      description: '突出动作场面和节奏感',
      icon: '⚡',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写动作导向风格的简介。

任务：为第 ${content.episodeNumber} 集撰写动作导向风格的简介
