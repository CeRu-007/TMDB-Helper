...
import {
  ...
  GenerationConfig,
  GenerationResult,
} from '../components/subtitle-episode-generator/types';
 ...
 */
export function convertToEpisodeContent(
 ...
 * 使用插件构建提示词
 */
export function buildPromptWithPlugin(
  episode: SubtitleEpisode,
...
export function getPluginStats() {
  ...
  return {
    ...
    enhanceOperations: pluginManager.getByType(PluginType.EnhanceOperation).length,
 ...
 * 获取增强操作的配置
 */
export function getEnhanceOperationConfig(operationType: string): {
  temperature: number;
  maxTokens: number;
} {
  const pluginManager = getPluginManager();
