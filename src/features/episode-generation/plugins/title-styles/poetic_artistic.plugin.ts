/**
 * 诗意文艺风格插件
 * 采用优美诗意的表达，注重意境和美感，如：月下花前、春风化雨、岁月如歌
 */

import { BasePlugin, PluginType, ITitleStylePlugin, EpisodeContent, ParsedTitle, TitleStyleConfig  } from '../core'

export const poetic_artisticPlugin: ITitleStylePlugin = new (class extends BasePlugin implements ITitleStylePlugin {
  constructor() {
    super({
      id: 'poetic_artistic',
      type: PluginType.TitleStyle,
      name: '诗意文艺',
      description: '采用优美诗意的表达，注重意境和美感，如：月下花前、春风化雨、岁月如歌',
      icon: '🌸',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['poetic_artistic', 'builtin'],
      metadata: {
        category: 'title',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写诗意文艺风格的标题。

任务：为第 ${content.episodeNumber} 集撰写诗意文艺风格的标题
