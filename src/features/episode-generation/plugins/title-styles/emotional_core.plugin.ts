...
export const emotional_corePlugin: ITitleStylePlugin = new (class extends BasePlugin implements ITitleStylePlugin {
  constructor() {
    super({
      id: 'emotional_core',
      type: PluginType.TitleStyle,
      name: '情感核心',
      description: '以情感冲突或情感高潮为标题主题，注重内心世界的描绘',
      icon: '💫',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['emotional_core', 'builtin'],
      metadata: {
        category: 'title',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写情感核心风格的标题。

任务：为第 ${content.episodeNumber} 集撰写情感核心风格的标题
