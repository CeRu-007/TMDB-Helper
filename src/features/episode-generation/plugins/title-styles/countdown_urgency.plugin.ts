/**
 * 紧迫倒计时风格插件
 * 营造紧迫感和时间压力，如：最后一战、倒计时、生死时速、关键时刻
 */

import { BasePlugin, PluginType, ITitleStylePlugin, EpisodeContent, ParsedTitle, TitleStyleConfig  } from '../core'

export const countdown_urgencyPlugin: ITitleStylePlugin = new (class extends BasePlugin implements ITitleStylePlugin {
  constructor() {
    super({
      id: 'countdown_urgency',
      type: PluginType.TitleStyle,
      name: '紧迫倒计时',
      description: '营造紧迫感和时间压力，如：最后一战、倒计时、生死时速、关键时刻',
      icon: '⏰',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['countdown_urgency', 'builtin'],
      metadata: {
        category: 'title',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写紧迫倒计时风格的标题。

任务：为第 ${content.episodeNumber} 集撰写紧迫倒计时风格的标题
