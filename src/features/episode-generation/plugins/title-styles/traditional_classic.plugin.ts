/**
 * 传统经典风格插件
 * 采用经典的命名方式，使用传统文学色彩的词汇，如：风云变幻、英雄本色、江湖恩仇
 */
...
export const traditional_classicPlugin: ITitleStylePlugin = new (class extends BasePlugin implements ITitleStylePlugin {
  constructor() {
    super({
      id: 'traditional_classic',
      type: PluginType.TitleStyle,
      name: '传统经典',
      description: '采用经典的命名方式，使用传统文学色彩的词汇，如：风云变幻、英雄本色、江湖恩仇',
      icon: '📜',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['traditional_classic', 'builtin'],
      metadata: {
        category: 'title',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写传统经典风格的标题。

任务：为第 ${content.episodeNumber} 集撰写传统经典风格的标题
