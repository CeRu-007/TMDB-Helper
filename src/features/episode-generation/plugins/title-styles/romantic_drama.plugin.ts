/**
 * 浪漫情感风格插件
 * 突出爱情线和情感纠葛，使用温馨或戏剧化的表达，如：心动时刻、告白之夜、离别之痛
 */
...
export const romantic_dramaPlugin: ITitleStylePlugin = new (class extends BasePlugin implements ITitleStylePlugin {
  constructor() {
    super({
      id: 'romantic_drama',
      type: PluginType.TitleStyle,
      name: '浪漫情感',
      description: '突出爱情线和情感纠葛，使用温馨或戏剧化的表达，如：心动时刻、告白之夜、离别之痛',
      icon: '💕',
      version: '1.0.0',
      author: 'TMDB-Helper',
      isBuiltin: true,
      tags: ['romantic_drama', 'builtin'],
      metadata: {
        category: 'title',
        difficulty: 'medium'
      }
  ...
  buildPrompt(content: EpisodeContent, options?: Record<string, any>): string {
    ...
    return `你是一位专业的影视内容编辑，擅长撰写浪漫情感风格的标题。

任务：为第 ${content.episodeNumber} 集撰写浪漫情感风格的标题
