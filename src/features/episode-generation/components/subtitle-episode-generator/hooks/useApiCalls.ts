...
import { SubtitleEpisode, GenerationResult, GenerationConfig } from '../types';
import { useScenarioModels } from '@/lib/hooks/useScenarioModels';
import {
  buildPromptWithPlugin,
  parseResultWithPlugin,
  initializePluginSystem,
  buildEnhancePromptWithPlugin,
  parseEnhanceResultWithPlugin,
  getEnhanceOperationConfig,
  getAllSummaryStyles,
...
export function useApiCalls(config: GenerationConfig) {
  ...
  // 调用API生成内容（为单个风格生成）
  const generateEpisodeContentForStyle = useCallback(
    async (episode: SubtitleEpisode, styleId: string): Promise<ApiCallResult> => {
        ...
        try {
          ...
          if (
            ...
            if (response.status === 401) {
                ...
                if (error && typeof error === 'object') {
                  ...
                  if ((error as any).details) {
                    if (typeof (error as any).details === 'string') {
                      return (
                        (error as any).details.toLowerCase().includes("exceeded today's quota") ||
                        (error as any).details.toLowerCase().includes('quota exceeded')
                      );
                    }

                    if (typeof (error as any).details === 'object') {
                      const detailsStr = JSON.stringify((error as any).details).toLowerCase();
                      return (
                        detailsStr.includes("exceeded today's quota") ||
              ...
              const isInsufficientBalanceError = (error: unknown): boolean => {
                ...
                if (error && typeof error === 'object') {
                  ...
                  return (
                    ...
                    (error as any).errorType === 'INSUFFICIENT_BALANCE'
                  );
                }
              ...
              if (
                isInsufficientBalanceError(errorData) ||
                isInsufficientBalanceError(responseText)
              ) {
                // 获取风格名称
                const allStyles = getAllSummaryStyles();
                const style = styleId ? allStyles.find((s) => s.id === styleId) : null;
          ...
          errorMessage = `网络错误或响应格式异常: ${(e as Error).message}`;
        }
  ...
  const enhanceContent = useCallback(
    async (
      ...
      if (operation === 'rewrite' && selectedTextInfo) {
        ...
        systemContent =
          '你是一位专业的文字编辑专家，擅长改写和优化文字表达。请严格按照用户要求进行改写，保持原意的同时提升表达质量。';
      } else {
        // 使用插件系统构建提示词
        const result = { generatedTitle: '', generatedSummary: content } as GenerationResult;
        prompt = buildEnhancePromptWithPlugin(
