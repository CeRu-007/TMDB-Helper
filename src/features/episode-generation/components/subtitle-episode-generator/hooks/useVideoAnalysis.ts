...
export function useVideoAnalysis(
  ...
  // 重新生成结构化内容（只生成SRT格式）
  const regenerateStructuredContent = useCallback(
    (updatedResult: VideoAnalysisResult): VideoAnalysisResult => {
  ...
  const handleGenerateEpisodeFromVideo = useCallback(
    (
      ...
      setSubtitleFiles: React.Dispatch<React.SetStateAction<SubtitleFile[]>>,
