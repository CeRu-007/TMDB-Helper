...
import {
  cleanTitleBrackets,
  cleanSummaryPrefixAndBrackets,
} from '@/features/episode-generation/lib/text-cleaner';
...
function isInsufficientBalanceError(error: unknown): boolean {
  ...
  if (error && typeof error === 'object') {
    ...
    return (
      balanceIndicators.some((indicator) => errorStr.includes(indicator)) ||
...
export function useContentGeneration(
  ...
  const [aiImprovingIndex, setAiImprovingIndex] = useState<{
    fileId: string;
    resultIndex: number;
  } | null>(null);
  ...
  const generateEpisodeContent = useCallback(
    async (episode: SubtitleEpisode): Promise<GenerationResult[]> => {
      const results: GenerationResult[] = [];
      ...
      for (const styleId of validSelectedStyles) {
        try {
          ...
          if (
            validSelectedStyles.length > 1 &&
            styleId !== validSelectedStyles[validSelectedStyles.length - 1]
          ) {
  ...
  const handleBatchGenerateAll = useCallback(
    async (
      setSubtitleFiles: (
        files: SubtitleFile[] | ((prev: SubtitleFile[]) => SubtitleFile[])
      ) => void,
      ...
      setSubtitleFiles(
        (prev: SubtitleFile[]) =>
          prev.map((file: SubtitleFile) => ({
            ...file,
            generationStatus: validFiles.some((vf) => vf.id === file.id)
      ...
      try {
        ...
        logger.info(
          `批量生成完成: 总计 ${totalGenerated} 个，成功 ${successfulGenerated} 个，失败 ${totalGenerated - successfulGenerated} 个`
        );
        ...
        if (!selectedFile && validFiles.length > 0) {
          ...
          const firstFileWithResults = validFiles.find(
            (file) => (allResults[file.id] || []).length > 0
          );
    ...
    async (
      ...
      try {
        ...
        if (operation === 'rewrite' && selectedTextInfo) {
          const newSummary =
            result.generatedSummary.substring(0, selectedTextInfo.start) +
            enhancedContent +
        ...
        if (isInsufficientBalanceError(error)) {
          ...
          toast({
            title: '错误',
            description: `${getOperationName(operation)}失败：${error instanceof Error ? error.message : '未知错误'}`,
            variant: 'destructive',
          });
        }
      }
    },
    [generationResults, enhanceContent, toast]
  );

  function getOperationConfig(operation: EnhanceOperation): {
    temperature: number;
    maxTokens: number;
  } {
    const configs = {
  ...
  const handleAIImprovement = useCallback(
    async (fileId: string, resultIndex: number, userPrompt: string) => {
      const results = generationResults[fileId] || [];
      const result = results[resultIndex];
      if (!result) {
        return;
...
简介：[新的简介内容]`;
  ...
  const handleUpdateResult = useCallback(
    (fileId: string, resultIndex: number, updatedResult: Partial<any>) => {
      setGenerationResults((prev: Record<string, GenerationResult[]>) => {
        const fileResults = prev[fileId] || [];
        const newResults = [...fileResults];
        if (newResults[resultIndex]) {
  ...
  const handleMoveToTop = useCallback((fileId: string, resultIndex: number) => {
    setGenerationResults((prev: Record<string, GenerationResult[]>) => {
      const fileResults = prev[fileId] || [];
      if (resultIndex <= 0 || resultIndex >= fileResults.length) {
        return prev;
      }
  ...
  const handleBatchExportToTMDB = useCallback(
    async (exportConfig: {
      ...
      try {
        ...
        for (const file of subtitleFiles) {
          ...
          for (const [, result] of episodeGroups) {
            const originalEpisode = file.episodes.find(
              (ep) => ep.episodeNumber === result.episodeNumber
            );
            const runtime = originalEpisode?.lastTimestamp
              ? timestampToMinutes(originalEpisode.lastTimestamp)
              : 0;
