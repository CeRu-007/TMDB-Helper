...
export function useConfigManagement() {
      ...
      try {
        ...
        try {
          ...
          const episodeResponse = await fetch(
            '/api/model-service/scenario?scenario=episode_generation'
          );
          const episodeResult = await episodeResponse.json();

          if (
            episodeResult.success &&
            episodeResult.scenario &&
            episodeResult.scenario.primaryModelId
          ) {
          ...
          if (
            ...
            speechResult.scenario &&
            speechResult.scenario.primaryModelId
          ) {
        ...
        // 兼容旧的配置存储方式
        const provider =
          (await ClientConfigManager.getItem('episode_generator_api_provider')) || 'siliconflow';
        const settingsKey =
          provider === 'siliconflow' ? 'siliconflow_api_settings' : 'modelscope_api_settings';
        ...
        if (episodeGenerationModel === 'deepseek-ai/DeepSeek-V2.5') {
          if (settingsText) {
            try {
              ...
              if (s.episodeGenerationModel) {
                episodeGenerationModel = s.episodeGenerationModel;
              }
            } catch {}
          }
        }
        ...
        if (saved) {
          try {
            ...
            if (parsed.selectedStyles && Array.isArray(parsed.selectedStyles)) {
              ...
              parsed.selectedStyles = parsed.selectedStyles.filter((id: string) =>
                validStyleIds.includes(id)
              );
            } else {
            ...
            const completeConfig = {
              ...
              includeOriginalTitle:
                parsed.includeOriginalTitle !== undefined ? parsed.includeOriginalTitle : true,
  ...
  useEffect(() => {
    const updateModelForProvider = async () => {
      const settingsKey =
        apiProvider === 'siliconflow' ? 'siliconflow_api_settings' : 'modelscope_api_settings';
  ...
  useEffect(() => {
    ...
    const handleGlobalSettingsClose = (
      shouldReopenSettingsDialog: boolean,
    ...
    window.addEventListener('global-settings-closed', ((event: CustomEvent) => {
      handleGlobalSettingsClose(
        event.detail?.shouldReopenSettingsDialog,
        event.detail?.setShowSettingsDialog
      );
    }) as EventListener);
