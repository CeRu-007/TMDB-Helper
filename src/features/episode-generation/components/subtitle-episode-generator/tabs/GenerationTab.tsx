                  ...
                  <SelectItem key={model.id} value={model.id}>
                    <div className="flex items-center gap-2">
                      ...
                      {model.id === scenarioModels.primaryModelId && (
                        <Badge variant="secondary" className="text-xs">{t("generationTab.primaryModel")}</Badge>
                      )}
                    </div>
                  </SelectItem>
                ))
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-4">
            ...
            <Slider
              ...
              max={300} min={20} step={5} className="flex-1"
            />
          ...
          <div className="flex items-center space-x-4">
            ...
            <Slider
              ...
              max={400} min={config.summaryLength[0] + 10} step={5} className="flex-1"
            />
            <span className="text-sm font-medium w-12">{config.summaryLength[1]}{t("generationTab.charUnit")}</span>
          </div>
        </div>
      </div>
              ...
              <div className="flex flex-col space-y-1 mt-1">
                ...
                {config.imitateConfig?.sampleContent && config.imitateConfig.sampleContent.length > 0 && (
                  <p className="text-xs text-blue-600 dark:text-blue-400">
                    {t("generationTab.autoAdjusted", { min: config.summaryLength[0], max: config.summaryLength[1] })}
                  </p>
                )}
              </div>
            </div>
