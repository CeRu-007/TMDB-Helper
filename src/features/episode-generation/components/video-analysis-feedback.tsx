        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-4">
          ...
          {currentStep && (
            <p className="text-xs text-blue-600 dark:text-blue-400 mt-2">
              {currentStep.message || `${t("emptyState.analyzing")}${stepNames[currentStep.id] || currentStep.name}...`}
            </p>
          )}
        </div>
      )}
        ...
        <div className="bg-green-50 dark:bg-green-950/30 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-green-600 dark:text-green-400">{t("feedback.detectionStatus")}</span>
              <Badge variant={subtitleInfo.hasSubtitles ? "default" : "secondary"}>
                {subtitleInfo.hasSubtitles ? t("feedback.subtitleDetected") : t("feedback.noSubtitleDetected")}
              </Badge>
            </div>
            {subtitleInfo.hasSubtitles && (
              <>
                <div className="flex items-center justify-between">
                  <span className="text-green-600 dark:text-green-400">{t("feedback.language")}</span>
                  <span className="text-green-700 dark:text-green-300">{subtitleInfo.language}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-green-600 dark:text-green-400">{t("feedback.confidence")}</span>
                  <span className="text-green-700 dark:text-green-300">{(subtitleInfo.confidence * 100).toFixed(1)}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-green-600 dark:text-green-400">{t("feedback.segmentCount")}</span>
                  <span className="text-green-700 dark:text-green-300">{subtitleInfo.segmentCount}</span>
                </div>
                {subtitleInfo.content && (
                  <div className="mt-3">
                    <span className="text-green-600 dark:text-green-400 text-xs">{t("feedback.contentPreview")}</span>
...
export function updateStepStatus(
  ...
  return steps.map(step =>
    step.id === stepId
      ? { ...step, status, message, progress } as VideoAnalysisStep
      : step
  );
}
