...
// 标题生成防抖和重试机制
const titleGenerationCache = new Map<
  string,
  {
    lastAttempt: number;
    attempts: number;
...
export const useAiChatHistory = (
  ...
  const updateCurrentChat = useCallback(
    async (newMessages: Message[], chatId?: string) => {
  ...
  const generateChatTitle = async (chatId: string, messages: Message[]) => {
    try {
      ...
      if (cache && now - cache.lastAttempt < TITLE_GENERATION_COOLDOWN) {
        logger.info('标题生成冷却中，跳过', {
          chatId,
          remainingTime: TITLE_GENERATION_COOLDOWN - (now - cache.lastAttempt),
        });
        return;
      }
      ...
      logger.info('开始生成对话标题', {
        ...
        attempt: titleGenerationCache.get(chatId)?.attempts,
      });
      ...
      // 从AI回复消息中获取使用的模型ID
      const usedModelId = (firstAssistantMessage as any).modelId;
      ...
      if (!targetModelId) {
        logger.error('无法确定目标模型ID', {
          chatId,
          usedModelId,
          primaryModelId: modelServiceData.scenario?.primaryModelId,
        });
        titleGenerationCache.set(chatId, {
          lastAttempt: now,
          attempts: cache?.attempts || 0,
      ...
      const targetModel = modelServiceData.models?.find(
        (m: any) => m.id === targetModelId || m.modelId === targetModelId
      );
      if (!targetModel) {
        logger.error('找不到目标模型', {
          chatId,
          targetModelId,
          availableModels: modelServiceData.models?.map((m: any) => ({
            id: m.id,
            modelId: m.modelId,
          })),
        });
      ...
      const provider = modelServiceData.providers?.find(
        (p: any) => p.id === targetModel?.providerId
      );
      if (!provider) {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          logger.warn('标题生成API调用频率过高，将在冷却后重试', {
            chatId,
      ...
      if (data.success && data.data?.title) {
        ...
        setChatHistories((prev) =>
          prev.map((chat) => (chat.id === chatId ? { ...chat, title: data.data.title } : chat))
        );
      } else {
