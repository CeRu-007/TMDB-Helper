    ...
    >
        ...
        <div className="mb-2 flex flex-nowrap gap-1 overflow-x-auto scrollbar-hide">
          {item.status === 'completed' ? (
            <Badge className="bg-gray-600 text-white text-xs px-2 py-1 rounded-full whitespace-nowrap">
              {getCompletionDate()
                ? `${getCompletionDate()} ${t('completedShort', { ns: 'media' })}`
                : t('completed', { ns: 'media' })}
            </Badge>
          ) : (
            <>
              {isDailyUpdate ? (
                <Badge
                  className={`text-white text-xs px-2 py-1 rounded-full flex items-center whitespace-nowrap ${scheduleTask?.enabled ? 'bg-purple-500' : 'bg-blue-500'}`}
                >
                  {scheduleTask?.enabled ? (
                    <>
                      <Clock className="h-3 w-3 mr-1" />
                      {t('daily', { ns: 'media' })} {getTimeOnly()} <span className="mx-1">·</span>{' '}
                ...
                <Badge
                  className={`text-white text-xs px-2 py-1 rounded-full flex items-center whitespace-nowrap ${scheduleTask?.enabled ? 'bg-purple-500' : item.weekday === new Date().getDay() || (hasSecondWeekday && item.secondWeekday === new Date().getDay()) ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}
                >
                  {scheduleTask?.enabled ? (
                    <>
                      <Clock className="h-3 w-3 mr-1" />
                      {getAirTimeDisplay()} <span className="mx-1">·</span>{' '}
                      {getTimeFromCron(scheduleTask.cron)}
                    </>
                  ) : (
                    getAirTimeDisplay()
      ...
      <div
        className={`relative aspect-[2/3] overflow-hidden rounded-lg shadow-md transition-all duration-150 ${
          isClicked ? 'scale-95 brightness-110' : 'group-hover:scale-[1.02] group-hover:shadow-xl'
        }`}
      ...
      >
        ...
        <CachedImage
          src={
            !imageError ? posterUrl : `${TMDB_IMAGE_BASE_URL}/${TMDB_POSTER_SIZE}/placeholder.jpg`
          }
          alt={item.title}
