 ...
 * 获取选择区域的数据
 */
export function getSelectedCellsData(data: CSVData, selectedCells: CellPosition[]): string[][] {
  return selectedCells.map(({ row, col }) => {
    if (row >= 0 && row < data.rows.length && col >= 0 && col < data.headers.length) {
...
export function updateSelectedCellsData(
  ...
  selectedCells.forEach(({ row, col }, index) => {
    if (row >= 0 && row < result.rows.length && col >= 0 && col < result.headers.length) {
...
export function insertRow(
  ...
  result.rows = [...result.rows.slice(0, insertIndex), newRow, ...result.rows.slice(insertIndex)];
 ...
 * 移动行
 */
export function moveRow(data: CSVData, fromIndex: number, direction: 'up' | 'down'): CSVData {
  const result = cloneCSVData(data);
 ...
 * 移动列
 */
export function moveColumn(data: CSVData, fromIndex: number, direction: 'left' | 'right'): CSVData {
  const result = cloneCSVData(data);
...
export function getAllColumnData(
  ...
  const colIndex = data.headers.findIndex((h) => h.toLowerCase() === columnName.toLowerCase());
  if (colIndex === -1) {
    return [];
  }
 ...
 * 计算最大 episode_number
 */
export function getMaxEpisodeNumber(data: CSVData, episodeColumnIndex: number): number {
 ...
 * 查找列索引
 */
export function findColumnIndex(data: CSVData, columnName: string): number {
  return data.headers.findIndex((header: any) => header.toLowerCase() === columnName.toLowerCase());
}

/**
 * 计算选择区域
 */
export function calculateSelectionArea(start: CellPosition, end: CellPosition): CellPosition[] {
