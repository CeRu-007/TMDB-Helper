...
export function isValidCSVData(data: unknown): boolean {
  ...
  if (!(csvData.headers as any[]).every((h: any) => typeof h === 'string')) {
    return false;
  }
  ...
  const validRows = csvData.rows as any[];
  if (
    !validRows.every(
  ...
  // 验证每行的列数是否与headers一致
  if (!validRows.every((row: any) => (row as any[]).length === (csvData.headers as any[]).length)) {
    return false;
  }
