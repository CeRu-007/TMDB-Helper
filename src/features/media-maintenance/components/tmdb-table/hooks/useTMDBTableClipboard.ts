...
import type { UseTMDBTableClipboardOptions, UseTMDBTableClipboardReturn } from '../types';
import {
  copyToClipboard,
  pasteFromClipboard,
...
export function useTMDBTableClipboard({
  ...
  const paste = useCallback(async (): Promise<boolean> => {
    ...
    // 应用粘贴数据
    const newRows = applyClipboardData(localData.rows, clipboardData, minRow, minCol);
    ...
    clipboardData.forEach((row, rowIndex) => {
      row.forEach((cell, colIndex) => {
        ...
        if (targetRow < localData.rows.length && targetCol < localData.headers.length) {
          updates.push({
            row: targetRow,
            col: targetCol,
            value: cell,
