...
function getDrawerPosition(cardEl: HTMLDivElement): {
  top: number;
  left: number;
  maxHeight: number;
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-700">
        <div className="flex items-center">
          ...
          <h3 className="text-sm font-semibold text-gray-100">
            {t('scheduleManagement', { ns: 'schedule' })}
          </h3>
        </div>
        <Button
          variant="ghost"
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-xs text-gray-400">
                      {t('executionTime', { ns: 'schedule' })}
                    </Label>
                    <Badge variant="secondary" className="bg-purple-500/20 text-purple-300 text-xs">
                      {getTimeFromCron(task.cron)}
                    </Badge>
                  </div>
                  <p className="text-xs text-gray-500">{getCronDescription(task.cron)}</p>
                </div>

                {task.nextRunAt && (
                  <div className="space-y-1">
                    <Label className="text-xs text-gray-400">
                      {t('nextRun', { ns: 'schedule' })}
                    </Label>
