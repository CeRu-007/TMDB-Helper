...
export function useSeasonActions({
  ...
  const getCurrentSeason = useCallback(() => {
    if (!localItem.seasons || !selectedSeason) {
      return null;
    }
  ...
  // 处理季数调整
  const handleSeasonAdjust = useCallback(
    (newSeason: number) => {
  ...
  const handleAddSeason = useCallback(
    (seasonNumber: number, episodeCount: number) => {
      ...
      // 检查季是否已存在
      const seasonExists = localItem.seasons?.some(
  ...
  const confirmDeleteSeason = useCallback(
    (seasonNumber: number) => {
      ...
      // 过滤掉要删除的季
      const updatedSeasons = localItem.seasons.filter(
        (season) => season.seasonNumber !== seasonNumber
      );

      // 更新扁平化的episodes数组，移除该季的所有集数
      const updatedEpisodes =
        localItem.episodes?.filter(
  ...
  const handleResetSeason = useCallback(() => {
    ...
    const updatedSeasons = localItem.seasons.map((season) => {
      if (season.seasonNumber === selectedSeason) {
        const resetEpisodes = season.episodes!.map((ep: any) => ({ ...ep, completed: false }));
  ...
  }, [selectedSeason, localItem, onLocalItemUpdate, onShowFeedback]);

  // 处理总集数变更
  const handleTotalEpisodesChange = useCallback(
    (newCount: number) => {
      if (!selectedSeason || !localItem.seasons) {
        return;
      }
