...
export function useEpisodeActions({
  ...
  const handleEpisodeToggle = useCallback(
    async (episodeNumber: number, completed: boolean, seasonNumber: number) => {
      // 计算要操作的集数范围
      ...
      if (isShiftPressed) {
        if (lastClickedEpisode === null) {
          ...
          onShowFeedback(`已选择起点：第${episodeNumber}集`, DELAY_1S);
          return;
        } else {
      ...
      // 更新状态
      const allCompleted =
        updatedItem.episodes?.every((ep) => ep.completed) && updatedItem.episodes.length > 0;
      ...
      if (!(
        ...
        lastClickedEpisode !== episodeNumber
      )) {
        setLastClickedEpisode(episodeNumber);
      }
    },
  ...
  const handleBatchToggle = useCallback(
    (seasonNumber: number, completed: boolean) => {
      ...
      const updatedSeasons = localItem.seasons.map((season) => {
        if (season.seasonNumber === seasonNumber) {
          const updatedEpisodes = season.episodes!.map((ep: any) => ({ ...ep, completed }));
          return { ...season, episodes: updatedEpisodes };
        }
        return season;
      });
  ...
  const handleMarkNextEpisode = useCallback(() => {
    ...
    // 找到第一个未完成的集数
    const nextEpisode = currentSeason.episodes.find((ep: any) => !ep.completed);
    if (!nextEpisode) {
