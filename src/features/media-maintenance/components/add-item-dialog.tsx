...
export default function AddItemDialog({
  ...
  const CATEGORIES = [
    {
      id: 'anime' as const,
      name: t('categoryNames.anime'),
      icon: <Sparkles className="h-4 w-4" strokeWidth={2} />,
    },
    {
      id: 'tv' as const,
      name: t('categoryNames.tv'),
      icon: <Tv className="h-4 w-4" strokeWidth={2} />,
    },
    {
      id: 'kids' as const,
      name: t('categoryNames.kids'),
      icon: <Baby className="h-4 w-4" strokeWidth={2} />,
    },
    {
      id: 'variety' as const,
      name: t('categoryNames.variety'),
      icon: <Popcorn className="h-4 w-4" strokeWidth={2} />,
    },
    {
      id: 'short' as const,
      name: t('categoryNames.short'),
      ...
      try {
        ...
        if (tmdbData) {
          ...
          if (tmdbData.seasons && tmdbData.seasons.length > 0) {
            calculatedTotalEpisodes = tmdbData.seasons.reduce(
              (sum: number, season: { totalEpisodes?: number }) =>
          ...
          setFormData((prev) => ({
            ...
            platformUrls:
              prev.platformUrls.length > 0 ? prev.platformUrls : tmdbData.platformUrls || [],
  ...
  useEffect(() => {
    if (open && prefilledData) {
      const { id, title, mediaType, posterPath, releaseDate, overview, voteAverage } =
        prefilledData;
  ...
  }, [open, prefilledData, handleSelectResult]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.category) {
      toast({
        ...
        description: t('independentPage.addItem.errors.selectCategoryHint', {
          ns: 'nav.maintenance',
        }),
        variant: 'destructive',
      });
    ...
    if (formData.totalEpisodes < 1) {
      toast({
        ...
        description: t('independentPage.addItem.errors.invalidEpisodesHint', {
          ns: 'nav.maintenance',
        }),
        variant: 'destructive',
      });
    ...
    try {
      ...
      if (!tmdbData) {
        throw new Error(
          t('independentPage.addItem.errors.getDetailFailed', { ns: 'nav.maintenance' })
        );
      }
      ...
      const missingImages = [];
      if (!tmdbData.logoUrl) {
        missingImages.push('logo');
      }
      if (!tmdbData.backdropUrl) {
        missingImages.push('backdrop');
      }
      if (!tmdbData.networkLogoUrl && selectedResult.media_type === 'tv') {
        missingImages.push('networkLogo');
      }
      ...
      if (tmdbData.seasons && tmdbData.seasons.length > 0) {
        seasons = tmdbData.seasons.map((seasonData: any) => ({
        ...
        const tmdbTotalEpisodes =
          tmdbData.totalEpisodes || seasons.reduce((sum, s) => sum + s.totalEpisodes, 0);
        ...
        if (isManualTotalEpisodes && formData.totalEpisodes !== tmdbTotalEpisodes) {
          ...
          if (seasons.length === 1) {
            ...
            const otherSeasonsTotal = seasons
              .slice(0, -1)
      ...
      const finalPlatformUrls =
        formData.platformUrls.length > 0 ? formData.platformUrls : tmdbData.platformUrls || [];

      // 判断是否手动设置了集数
      const isManuallySetEpisodes =
      ...
      if (isDuplicate) {
        ...
        toast({
          ...
          description: t('independentPage.addItem.errors.alreadyExistsDesc', {
            ns: 'nav.maintenance',
            title: newItem.title,
          }),
          variant: 'destructive',
      ...
      toast({
        ...
        description: t('independentPage.addItem.errors.addSuccessDesc', {
          ns: 'nav.maintenance',
          title: newItem.title,
        }),
        variant: 'default',
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : t('independentPage.addItem.errors.networkError', { ns: 'nav.maintenance' }),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
