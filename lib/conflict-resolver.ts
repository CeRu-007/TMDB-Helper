...
export class ConflictResolver {
  ...
  private resolveByStaggering(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // 确保不会调整到过去的时间
      if (newTime.getTime() > Date.now() && this.isTimeSlotAvailable(newTime, conflicts)) {
        return {
          taskId: task.id,
          taskName: task.name,
          originalTime: targetTime,
  ...
  private resolveByPriority(
    ...
    // 检查是否有低优先级的冲突任务
    const lowerPriorityConflicts = conflicts.filter(conflict => {
      // 这里需要获取冲突任务的优先级，简化处理
      return conflict.severity === 'low' || conflict.conflictType === 'priority';
    });
   ...
   * 计算队列位置
   */
  private calculateQueuePosition(taskPriority: number, conflicts: ConflictInfo[]): number {
    // 基于优先级和冲突数量计算队列位置
  ...
  public getResolutionStats(): {
    ...
    return {
      ...
      strategyCounts,
      averageAdjustmentMs: adjustmentCount > 0 ? totalAdjustment / adjustmentCount : 0,
      successRate: total > 0 ? (this.resolutionHistory.filter(r => r.newTime || r.queuePosition).length / total) : 0
    };
  }
}
